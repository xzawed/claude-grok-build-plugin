...
**`docs/10-service-audit-queue.md`의 열린 항목.** 게이트는 `.claude/skills/repo-scope`.
요청되지 않은 작업은 시작하지 않는다 — 큐가 비면 기본값은 다시 "없음"이고, 새 기능은 사용자가
목표·done을 명시한 뒤에만. 잔여 분류는 `docs/09`, **고치는 절차는 루트 `CLAUDE.md`**의
"작업 수행 방법".
