...
- **최신 릴리스 `v0.2.23`** (2026-09-06). 무엇이 왜 나갔는지는 `docs/releases/`와
...
- **다음 할 일 (이 레포): 없음.** 감사 세 번이 연 25건이 전부 닫혔다 — 2026-09-05 기능 감사 20건
  ...
  **광고하지 않은 경로로도 계약이 지켜지는가**. 마지막 질문이 잡은 게 A24다(값을 붙여 쓴 `-p`가
  ...
  적는다 (**번호는 재사용하지 않는다** — A25까지 썼으므로 다음은 A26이다).
  범위 밖(외부/수동/보류)은 `docs/09`이고, 기각·반증된 항목을 다시 제기하기 전에는
  `docs/09`·`docs/releases/`의 근거부터 읽을 것.
- **사람이 해야 할 미해결: 없음.** `v0.2.23`은 머지 직후 태그·릴리스했고, 클론→설치본을 갱신한 뒤
  캐시 번들을 `accept-release.mjs`로 채점해 **10/10**, 이어서 실제 Grok 위임까지 돌려 수락했다
  (`billing: subscription` · `filesChanged` · **커밋 없음**). 실행 기록은 `docs/09` §5.
  **다음 세션이 할 일은 한 줄뿐이다** — `grok_build_status`의 `serverVersion`이
  `mcp-server/package.json`과 같은지 보고 `docs/09` §5에 한 줄 남기기.
  ⚠️ 왜 그게 다음 세션 몫인지 기억할 것: 세션은 **자기가 시작할 때의 MCP 프로세스를 물고 있다.**
  갱신을 수행한 세션은 갱신 뒤에도 옛 번호를 말한다 — v0.2.23 런에서 실측됐다(설치본·캐시는
  `0.2.23`인데 그 세션의 `grok_build_status`는 `0.2.22`). 그러니 **갱신 뒤 새로 시작된 세션**이
  `grok_build_status`(= `/grok:status`의 구동부, `commands/status.md`)를 한 번 부르면 그것이
  증거다. 사람을 기다릴 필요는 없다.
