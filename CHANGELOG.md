...
그래서 위임받은 워커마다 이 서버의 사본이 떠 있었고(기록 923개 중 923개), 워커는 그것으로 또 다른
grok을 띄울 수 있었다. 재현하니 바깥 delegate는 `filesChanged: []`를 돌려줬고, 그 사이 중첩 런이
다른 디렉터리에 파일을 썼다. **실사용에서도 세 번 시도됐다** — 리뷰형 프롬프트를
`grok_build_verify`로 다시 넘기려 한 것이고, 그중 한 번은 worktree 격리 런에서 본 저장소를
지목했다. 셋 다 plan 런이라 grok의 권한 게이트가 취소했을 뿐이고, `--always-approve` 런은 MCP
호출을 실행한다(9/9).

**고친 방법:** `buildGrokEnv`가 모든 grok에 `GROK_BUILD_WORKER=1`을 붙이고, 그 env를 물려받은 사본은
9개 도구를 목록에 둔 채 전부 거절한다(spawn·이력 없음). grok의 끄는 스위치를 쓰지 않은 이유:
찾은 것(`GROK_CLAUDE_MCPS_ENABLED` 등, 비공개)은 `~/.claude.json` 서버만 끄고 플러그인 출처 서버는
그대로 둔다(실측). 숨기지 않고 거절하는 이유: 도구가 없으면 워커가 10턴·38만 토큰을 들여 찾았다.
...
바이트 8192에서 잘라 읽는다. 우리 `tools/list`가 12,625바이트라 세션의 약 35%에서 이 서버 연결이
`timed out after 70s`로 실패했다 — 그게 A34를 간헐적으로 가렸고, 재현이 두 번 실패한 이유다.

⚠️ **하네스가 다섯 번 틀렸다 (진짜 결함 1건 대비).** 전부 출력이 명백히 이상해서 잡혔다.
...
`grok_build`라 모든 도구 호출에 붙어 있었다. `use_tool`의 `tool_name`으로 좁히니 4건.
