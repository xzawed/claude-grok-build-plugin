...
**이 플러그인 레포의 의도된 제품 범위(다리 + 협업 표면 + first-mile + 소비자 계약/키트)는 완료다 (최신 릴리스 `v0.2.29`).**  
...
⚠️ **모듈별 감사 현황은 소스의 `FOUND BY GROK` 마커가 원천이다** — `grep -rln "FOUND BY GROK"
mcp-server/src/`. 여기 목록을 적어두면 낡는다. 2026-09-22 기준 아직 안 본 것: `config.ts`,
`status.ts`, `orchestrator.ts`, 그리고 `routing.ts`(설계 질문으로만 보여줬고 은닉성 감사는 아직).
