...
#106 머지 뒤 같은 질문을 한 번 더, **더 넓게** 물었다: `CLAUDE.md`가 이름 대는 것 중 실재하지
않는 것이 있나. 실측 — 경로 77개, 저장소 소유 식별자 38개, npm 스크립트 3개, tool 이름 10개,
구조 주장 17개(hook matcher 대 `hooks.json`, 버전 4곳 일치, 커맨드·스킬·에이전트·설계문서 실재),
그리고 산문 계약 25건(절대 원칙 + 컴포넌트 지도)을 소스와 대조했다. **전부 성립, 결함 0건.**
가장 놓치기 쉬운 것도 성립했다 — `dist/hook.js`는 9KB이고 `--always-approve`·`best_of_n`을
담고 있지 않다(`index.js`는 815KB). `prompt-flags.ts`를 리프로 뗀 이유가 실제로 지켜진다.

**그래서 남긴 것은 결함 수정이 아니라 하네스다.** `test/claude-md-claims.test.ts` 5건이
floor 재등장 금지 + 경로·식별자·스크립트·tool 실재를 고정한다. 오늘 아침 #106이 고친 드리프트는
**아무 테스트도 이 문서를 읽지 않아서** 하루를 살아남았다. 빨간불 확인: `^4.1.0`과 없는 파일·
식별자를 도로 심으니 세 건이 정확히 그 이름을 들어 실패했고, 되돌리니 녹색이다.

⚠️ **이 테스트가 덮지 못하는 것을 문서에도 적었다.** 이름이 있는지만 본다 — 실재하는 함수에
틀린 동작을 적어두면 그대로 통과한다. Grok 반증(`METHOD_WEAK`)이 후보 6개 중 바로 그것을
**가장 위험한 하나**로 지목했고, 동의한다. 나머지 판정은 실측으로 갈렸다: B4(부분문자열 매칭)는
선언 형태로 다시 재니 38개 전부 실제 선언·호출·리터럴로 해소돼 **실제 사례 0건**, B3(접두사 7개
OR)도 두 곳 이상에서 해소되는 토큰이 마켓플레이스 `source` 리터럴 `./` 하나뿐이라 역시 0건,
B5(개수 주장)는 `server-tools.test.ts`가 이미 "정확히 아홉 개"로 고정하고 있다.

⚠️ **하네스가 조용히 거짓말한 사례를 하나 더 겪었다.** 엄격 재검사 스크립트를 heredoc으로 쓰다
역슬래시가 한 겹 접히면서 템플릿 리터럴 안의 정규식이 무력화됐고, `buildGrokEnv`를 포함해 **25개가
"없음"으로 나왔다.** 명백히 틀린 결과라 파일을 열어 확인했기에 망정이지, 그럴듯한 숫자였다면 그대로
보고했을 것이다. 2026-09-12 앞 항목의 교훈과 같다 — **의심스러우면 하네스부터 의심한다.**

## 2026-09-06

### 수락 실행 기록 — v0.2.23 (릴리스 아님)

머지 → **즉시** 태그·릴리스 → 클론 갱신 → 설치본 `0.2.22 → 0.2.23` → 캐시 채점 순. 단계별 수치는
`docs/09` §5 "실행 기록 — v0.2.23"이 원천이다. 요약: `accept-release` **10/10**, 캐시 번들과 레포
`dist`가 바이트 동일, **A24가 배포물에서 실제로 닫혔음을 확인**(캐시의 `dist/hook.js`가 로그아웃
상태에서 `["-p2+2"]`·`["-p/tmp/x"]`·`["-pfoo.txt"]`를 전부 deny — 0.2.22에서는 셋 다 allow였다),
실제 Grok 위임 `completed`·`subscription`·`filesChanged: [a.txt]`·**커밋 없음**, 집계가 그 디렉터리를
정확히 지목(A25 경로).

이 런에서 **B4의 규칙이 자기 자신에게 재현됐다**: 갱신을 수행한 세션의 `grok_build_status`는
설치본·캐시가 `0.2.23`인데도 `serverVersion: 0.2.22`를 답했다. 그 세션이 시작할 때 띄운 프로세스가
그대로이기 때문이고, 그래서 GUI 경로 확인은 **다음 세션** 몫이다(사람 몫이 아니다).

릴리스 절차에서 **놓쳤다가 뒤늦게 잡은 것**: `CONTRIBUTING.md`가 "테스트가 없어 이 목록만이 잡는다"고
경고한 세 자리 중 두 자리(`mcp-server/package-lock.json` ×2, `docs/03-plugin-spec.md`)를 v0.2.23
범프 때 안 옮겼다. 태그는 규약대로 먼저 끊었고(사용자가 실행하는 번들은 이미 옳았다) 메타데이터는
이 커밋에서 맞췄다 — 그래서 `v0.2.23` 태그 트리의 lockfile에는 `0.2.22`가 남아 있다.

### v0.2.23 — 붙여 쓴 프롬프트도 프롬프트다 (릴리스+전체 코드 감사 A24·A25)

오너가 "현재 릴리즈된 내용과 전체 코드를 Grok과 함께 검증"을 요청해 돌린 감사다. 앞선 두 감사와
질문이 또 달랐다 — ①"기능이 도는가"(09-05) ②"광고한 계약이 지켜지는가"(09-06 전체 감사)에 이어
③**"광고하지 않은 경로로도 지켜지는가"**. ①②를 통과한 코드에서 ③이 두 건을 잡았다. 전문:
`docs/releases/v0.2.23.md`. 유닛 572 → 578.

- **A24** `-p`는 값을 **붙여서도** 받고 clap은 `-p2+2`를 `-p 2+2`와 똑같이 읽는다(실측: 두 형태에
  일부러 틀린 `--model`을 붙였더니 **같은 에러**가 돌아왔다 — grok이 붙인 토큰을 플래그로 받았다는
  뜻). 그런데 짧은 플래그 판정이 `/^-[A-Za-z][A-Za-z]+$/`, 즉 **토큰 전체가 알파벳**이어야 했다.
  붙인 값에 숫자·슬래시·점이 하나라도 있으면 **인증 게이트와 위임 이력에서 동시에** 사라졌다.
  `grok_cli`에는 서버 측 `checkAuth`가 없어 PreToolUse hook이 유일한 게이트인데(그 사실을
  `hook.ts`가 직접 적어뒀다) 그게 열려 있었다. 배포 `dist/hook.js` 실측: 로그아웃 상태에서
  `["-p","2+2"]`는 deny인데 `["-p2+2"]`는 **allow**. 배포 번들 실측: 그 호출이 `status: ok`로
  grok의 `text:"**4**"`·`stopReason: end_turn`·실제 sessionId를 받아왔는데(=유료 턴)
  `promptRun`은 부재였고 이력은 **2행 그대로**였다. A2가 고친 피해가 다른 문으로 재발해 있었다.
  판정 대상을 "토큰 전체"에서 **"앞쪽 옵션 문자열"**로 바꿨다 — 붙인 값의 `p`는 데이터지 옵션이
  아니므로(`-s01a0p619`) 과잉 게이트도 늘지 않는다.
  **게이트 구멍은 Grok이 먼저 찾았다**(`-p0`·`-p/tmp/x`·`-pfoo.txt` → `GAP_FOUND`). 수정 후 다시
  검토시켰을 때는 누락을 하나도 못 찾았고, 대신 `-mp`(=`--model p`) 과잉 게이트를 지적했는데
  **기각**했다 — 실측상 수정 전후가 동일하고, 기존 테스트와 주석이 이미 못박은 의도적 정책이다.
- **A25** `runGrokCli`는 생략된 cwd를 `process.cwd()`로 채우는데 이력을 쓰는 호출부는 그걸 몰라
  `cwd ?? ''`를 적었다. 실측: **같은 디렉터리에서 돈 위임 2건 중 1건만** 그 디렉터리로 집계됐다
  (전체 2 → 필터 1). A17이 읽는 쪽에서 고친 문제를 쓰는 쪽이 만들고 있었다. 호출부에서 기본값을
  다시 유도하는 대안은 기각했다 — A7이 정확히 그것 때문에 생겼다. 이제 **실행이 자기가 쓴
  디렉터리를 보고한다**(`GrokCliResult.cwd`, 필수 필드). Grok 판정 `FIX_SOUND`, 그 유일한
  공격(`cwd:""`)은 실측으로 반증했다(spawn 전 거부, 행 없음).

**결함이 아니라고 판정한 것:** 레닥션이 사람이 외우는 비밀번호를 못 가린다(`password = Password1!`).
Grok이 `LEAK_FOUND`를 냈으나 4건 중 2건은 **검토용 발췌본이 실제 키 목록보다 짧아서** 생긴
오탐이었고(실제 코드는 `OPENAI_API_KEY`·`GH_TOKEN` 모두 마스킹), 남은 2건은 `CLAUDE.md` 절대 원칙
4와 `docs/04`가 이미 "마스킹은 완화이지 보장이 아니다"로 명시한 한계다 — 하한을 낮추면 A6에서
Grok이 잡았던 과교정이 그대로 돌아온다.

**감사가 확인하고 넘어간 것:** 태그 `v0.2.22`의 dist와 `origin/main`의 dist가 바이트 동일(=캐시
채점이 곧 릴리스 채점) · 도구 9개 전부 등록·전부 `additionalProperties: false` · 커맨드 27·스킬 2·
에이전트 1이 참조하는 도구 이름 9개 전부 실재 · `hooks/hooks.json` 래핑 정상 · `accept-release` 10/10.

### 수락 실행 기록 — v0.2.22 · `docs/10` B4가 닫혔다 (릴리스 아님)

**남아 있던 마지막 한 칸이 사람 몫이 아니었다.** B4는 "재시작한 세션이 이 번들을 로드하는가"를
묻는 항목이고, v0.2.18부터 네 번의 릴리스에 걸쳐 "오너가 Claude Code를 재시작하고
`/grok:status`를 눌러야 한다"로 이월돼 왔다. 실제로 막혀 있던 것은 **그 세션**이었지 사람의
부재가 아니었다 — 세션은 자기가 시작할 때 띄운 MCP 프로세스를 물고 있으므로, 갱신 직후의 세션은
무엇을 눌러도 옛 번들을 채점한다. 반대로 **갱신 뒤 새로 시작된 세션은 자기 안에서 잰다.**
이 세션이 그 세션이었고, `grok_build_status`(= `/grok:status`의 몸통 한 줄, `commands/status.md`)가
`serverVersion: 0.2.22`를 돌려줬다. 캐시가 버전 키라 그 답은 `…/grok/0.2.22`에서만 나온다.

이어서 `docs/09` §5b의 나머지를 같은 세션에서 실행했다 — 5a 헤드리스 10/10, route, throwaway
cwd 실위임(`completed` · `subscription` · `filesChanged: [a.txt]`), diff는 `-hello/+hi` 한 줄,
`git log`는 init 하나(**커밋 없음**), 집계 성공률·구독 비중 100%. 단계별 수치는
**`docs/09` §5 "실행 기록 — v0.2.22"가 원천**이다.

결론은 Grok에게 반증시켰다(`CLAUDE.md` "5번 조리법", 5개 공격 → `CLAIM_SOUND`). 가장 날카로운
지적은 **"`claude plugin list`만으로는 못 닫는다"** — 설치된 버전과 세션이 말하고 있는 프로세스는
다르고, 닫는 것은 오직 세션 안에서 읽은 `serverVersion`이다.

그래서 세 곳을 고쳤다: `docs/10` B4 삭제(+ 다음은 B6), `docs/09` §5 실행 기록 추가,
그리고 `accept-release.mjs`의 마지막 줄 — "Still needs a human"이라고 인쇄하던 것이 이 유령
항목을 네 릴리스 동안 살려둔 문장이라, **다음 세션에게 시키는 문장**으로 바꿨다. 함께
`CLAUDE.md`에서 낡은 설치본 버전 서사(`0.2.17` / `0.2.20`)와 중복된 B4 블록을 걷어냈다.

### v0.2.22 — 광고한 계약을 실제로 지키게 한다 (전체 감사 A21~A23)

같은 날 v0.2.21이 기능 감사 큐를 비운 뒤 **문서·코드 전수 감사**를 돌렸다. 질문이 달랐다 —
"각 기능이 동작하는가"가 아니라 **"우리가 광고한 계약이 실제로 지켜지는가"**. 전문:
`docs/releases/v0.2.22.md`. 유닛 555 → 572.

- **A21** 9개 도구 전부가 `additionalProperties: false`를 광고하는데 **실제로 거부하던 건
  `grok_build_route` 하나뿐**이었다(A1이 `.strict()`를 붙인 그것). 나머지 8개는 미지의 키를 받고
  조용히 벗겨냈다 — 즉 필드 이름 오타가 그 필드 없이 성공으로 끝났다. throwaway repo에서
  `worktreee`(e 3개)로 delegate한 결과: `completed` · `worktreePath` 없음 · **grok이 사용자
  작업 디렉터리를 직접 편집**. 격리를 요청했는데 안 됐고, 응답은 성공이라 말했다. 수정 후 같은
  페이로드는 `unrecognized_keys: ["worktreee"]`로 거부되고 디렉터리에 아무것도 쓰이지 않는다.
  **안전 강등이 하나가 아니라 둘이라는 건 Grok이 반증해서 알았다** — `sandbox`도 같은 모양이고
  (`delegate.ts:391`이 실제로 플래그를 넘기며 Linux/macOS는 커널이 강제), 확인 결과 지적이 맞았다.
  오차단 위험도 실측했다: 스펙대로 `params._meta`에 실린 메타데이터는 통과하고 `arguments` 안에
  잘못 넣은 것만 거부된다. 문서화된 소비자(`examples/orchestrator-consumer.md`)가 보내는 6개
  필드도 전부 스키마 안이다. Grok 정적 검증: `.strict()`가 바꾸는 것은 미지의 키 하나뿐이고
  음수 `timeout_ms`·`null`은 전에도 후에도 똑같이 거부된다.
- **A22** `redactSecrets` 재측정(튜닝에 안 쓴 코퍼스): 시크릿 18종 중 16종 마스킹, prose 12종 중
  **0종 훼손** — A6 라운드에서 Grok이 잡았던 과교정은 완전히 사라진 상태였다. 새던 것은
  `npm_...` 하나. 30자 하한이 이 규칙의 전부다(`npm_config_registry` 류가 흔한 접두사라, 하한이
  없으면 그 과교정을 그대로 재현한다) — 환경변수 3종을 회귀 테스트로 박았다. 재측정 최종
  **leaked 0/17 · mangled 0/15**. 처음 둘로 보였던 것 중 `ssh-rsa` 공개키는 **내 시험 케이스가
  틀린 것**이라 기각하고, 대신 "공개키는 가리지 않는다"를 테스트로 못박았다.
- **A23** `prompt-flags.ts`가 CLAUDE.md 컴포넌트 지도에서 빠져 있었다. 장식이 아니라 hook 번들에
  delegation 엔진이 딸려 들어오지 않게 하려고 떼어낸 리프 모듈이다 — 한 줄이라 감사 기록 커밋에서
  바로 닫았다.

**감사가 확인한 것(다시 열지 않도록):** 도구 9·커맨드 27 수치 일치 · 모든 live 입력 필드가
`docs/04` 자기 섹션에 존재 · `/grok:<verb>` 참조 전부 실재 · **값 수준 검증은 원래 견고**
(`action` 진짜 enum, 타입·음수 거부 — 새던 건 미지의 키뿐) · 절대 원칙 10개 성립 ·
**계약 §6 재측정** — grok 1.0.13은 여전히 `--permission-mode plan`을 무시하고 파일을 썼다.
자동 검사 1차 24건은 전부 정규식 오탐이라 보고에서 뺐다(`grok_error`는 도구가 아니라 status 값,
끊긴 링크는 전부 "SHIPPED — historical record" 배너 안).

### v0.2.21 — 감사 큐 A7~A20 (큐가 비었다)

2026-09-05 감사가 연 20건 중 남아 있던 14건. 전부 배포 번들을 `.claude/tools/mcpcall.mjs`로
구동해 재현 → 실패하는 테스트 → 수정 → **같은 페이로드로 재측정** → Grok 반증 순으로 처리했다.
전문과 수치: `docs/releases/v0.2.21.md`. 유닛 530 → 555.

- **A7** hook의 subscription deny 분기가 **출하 구성에서만** 죽어 있었다. `.mcp.json`에 env가
  없어 `GROK_BUILD_AUTH_MODE`는 미설정이고, `resolveHookMode`는 그걸 `unknown`으로 읽었다 —
  그 분기를 짚는 테스트들이 전부 변수를 손으로 세팅해서 녹색이었던 것이 결함을 가렸다. 이제
  서버의 파서에 위임한다(정의를 하나로). 미설정 ALLOW→**DENY**, `Subscription`/` subscription `도
  마찬가지. 잘못된 값만 `unknown`으로 남는다 — 서버가 아예 못 뜨는 상태에서 "`grok login`을
  실행하세요"는 엉뚱한 안내다(A12).
- **A8** `worktree diff`가 한 응답 안에서 자기와 모순됐다(파일 3개 나열 + "1 file changed").
  `git diff`는 untracked를 못 보는데 그게 `--always-approve`의 주 산출물이다. 큐가 제안한
  `add -A`→`reset`은 창이 있는 **변형**이라 기각하고, `GIT_INDEX_FILE`로 worktree 밖 임시
  인덱스에 대고 같은 뷰를 만들었다 — 호출 전후 porcelain 바이트 동일. 테스트는 실제 git.
- **A9** 취소된 확인 프롬프트가 성공과 같은 모양이었다(`status ok`, exit 0). 유일한 근거가
  4000자 tail 안에서 잘릴 수 있는 산문이라, **잘리기 전 전체 출력**에서 판정해 `cancelled:true`로
  낸다. 두 마커를 모두 요구한다 — `sessions search cancelled`는 매칭 세션을 내고, `-y`로 응답된
  프롬프트도 같은 줄을 찍는다.
- **A10** `isError`가 양방향으로 틀렸다. `blocked`(spawn조차 안 됨)가 false로, api 모드 키 없는
  `status`가 13개 필드를 다 채워 놓고 true로 나갔다. `isError`는 "요청한 일이 일어났는가"에
  답한다 — blocked·cancelled는 true, 유효한 페이로드를 낸 읽기 전용 진단은 false.
  `grok_auth_check`는 **의도적으로** 그대로다(출력 전체가 판정이라 잃을 게 없다). 그 비대칭을
  테스트로 박았다.
- **A11** 한 단어 오타(`sesions`)가 60초를 태우고 읽을 수 없는 ANSI 프레임을 냈다 — 미지의 첫
  토큰은 grok에게 **프롬프트**이고 `-p` 없는 프롬프트는 TUI를 띄운다. `import`가 이미 그 이유로
  막혀 있던 것을 일반화. 20,729ms → **611ms**. allowlist라 낡는 방향이 반대여서, 슬롯이
  불확실하면 규칙이 물러서고 거부 문구가 "새 서브커맨드일 수 있다"고 직접 말한다.
- **A12** 잘못된 `GROK_BUILD_AUTH_MODE`가 9개 도구를 한꺼번에 죽이면서 로그에는 번들 라인 번호와
  6개 프레임을 남겼다. **빠르게 실패하는 것은 유지**(조용한 기본값 = 잘못된 과금 모드)하고 보고만
  한 줄로. 그 외 에러는 스택 유지 — 거기서는 프레임이 진단이다.
- **A13** api 모드가 `"not-a-real-key-at-all"`에도 "인증 준비됨"이라고 했다. 이 경로는 xAI에
  접속하지 않으므로 **검사하는 것(존재)을 말한다.** `ok`는 true로 남기고(존재는 공짜로 확인
  가능한 마지막 사실), 검증 호출도 넣지 않았다(하필 과금을 신경 쓰는 모드에서 왕복은 청구 대상).
  세션이 함께 있으면 "키가 거부되면 구독 세션으로 넘어간다"고 덧붙인다.
- **A14** `plan`이 `additionalProperties:false`를 광고하면서 `worktree`/`model` 등을 **거부 대신
  삭제**했다. 이제 verify와 같은 필드 집합을 받고 전달한다. `worktree:true` → 실제 격리,
  `model:"grok-code"` → **`grok_error`**(값이 정말 틀렸고 이제 호출자가 안다). plan에 격리가
  중요한 이유는 plan이 읽기 전용이 아니기 때문이다(1.0.13이 플래그를 무시).
- **A15** `inspect`(7,269자)에서 정확히 가치 있는 머리를 버리고 꼬리를 남겼다. help도 같다 —
  이 세션이 `grok --help`로 직접 겪었다. `inspect`/help는 head, 나머지는 tail, 어느 쪽인지는
  `stdoutKept`. `inspect --json`(~81KB)용으로 `max_chars`(상한 100,000)를 열었다.
- **A17** 이력 1,858행이 있는데도 cwd 스코프 0건이 "아직 위임 이력이 없습니다 · `/grok:setup`"으로
  나갔다 — 데이터 손실처럼 읽히고 멀쩡한 설치에 setup을 다시 돌리라고 보낸다. 필터 아래의 0과
  전체의 0은 다른 사실이므로 다르게 말한다. status 대시보드도 물려받는다.
- **A16 · A18 · A19 · A20** (문서) — `filesChanged`는 git 저장소 밖에서 항상 비어 있다는 사실을
  tour/setup에 명시 · logout이 일방통행임을 먼저 말하고 명시적 확인을 요구 · 배포 표면 6곳의 맨
  상대 경로를 `${CLAUDE_PLUGIN_ROOT}/docs/…`로(오너 판단) · 드리프트한 커맨드 문서 3곳을 1.0.13에
  **다시 물어보고** 수정(`mcp`에 실제로 enable/disable이 있다).

**Grok은 6/9 항목에서 확인, 1건에서 반증.** 반증은 A7의 "한 조치가 항상 게이트를 연다"는 내
주장이었고 문자 그대로 맞다(미설치 + 미로그인이면 두 단계). 번들로 사슬을 실측해 전제조건이
둘이면 조치도 둘인 게 맞음을 확인하고 **코드는 바꾸지 않았다** — 지적을 받되 행동 전에 재는
규칙 그대로다. timeout 3회는 전부 표가 넓거나 칸당 두 가지를 물었을 때였고, 줄이자 1분 안에
돌아왔다(루트 `CLAUDE.md` 조리법 재확인).

**큐의 A 섹션이 비었다.** `docs/10`은 지우지 않는다 — 재현 하네스와 B(측정 불가)·C(손대지 말 것)가
다음 감사에 필요하다. 다음 번호는 A21이다.
