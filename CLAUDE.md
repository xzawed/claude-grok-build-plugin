...
- **다음 할 일: 없음** (`.claude/skills/repo-scope`). 오너가 목표를 줄 때까지 기본값은 "없음"이다.
  ...
  적고 **번호는 재사용하지 않는다 — 다음은 A36이다.** 범위 밖(외부/수동/보류)은 `docs/09`.
...
- ⚠️ **코드를 *추적*하게 하는 주장 — 반례를 만들라는 것 포함 — 은 이 조리법으로도 캡을 넘었다**(2026-09-24: 새 추적
  프롬프트 6/6이 300~360초 timeout, grok이 명세를 웹에서 받아 와 추론을 이어갔다). **측정한 사실을 주고 경로마다
  *분류*하라는 주장은 답이 나왔다** — 4번 중 2번은 첫 실행에, 2번은 캡에 걸린 세션을 `resume`해 "이미 한 분석으로
  지금 verdict.md를 쓰라"고 하자 나왔고, 진짜 결함 둘(A35, worktree 가상 폴더)을 찾았다. 반례 탐색은 **독립 구현과의
  차등 비교**로 하고, Grok에는 후보 *제안*이나 분류를 맡긴다.
...
- `env.ts` — grok **대신** 홈을 찾는 곳은 `grokHomeFor(env, grok이 실행될 폴더)`로 묻는다(A35, 계약 §8). 상대
  `GROK_HOME`은 grok의 작업 폴더(`--cwd`가 이긴다) 기준이다 — worktree 위임은 새 worktree다(`newWorktreeStandIn`).
