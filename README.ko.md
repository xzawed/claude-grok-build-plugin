...
내부적으로 `mcp-server/`(TypeScript, ESM)가 8개 MCP tool을 stdio로 제공합니다 — auth, delegate, plan, verify, usage(insights), worktree, **route**(추천만), cli — PreToolUse 인증 hook. 유닛 테스트 144개, 사전 빌드 번들 커밋.
