...
| `/grok:status` shows **`grokHomeNote`**, or a delegation still says to log in after you did | `GROK_HOME` is a relative path. grok resolves it against the folder it runs in — the task folder, or a brand-new worktree with `worktree: true` — and does not expand `~`. So each folder has its own grok home, and a `grok login` run elsewhere landed in another one | Set `GROK_HOME` to an absolute path and restart Claude Code. If per-folder homes are what you want, run `grok login` from the task folder; worktree delegations can't use a relative home, since each new worktree has no session |
