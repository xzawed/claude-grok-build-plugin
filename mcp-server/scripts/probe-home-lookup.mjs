 ...
 *
 * Usage: npm run probe:home   (win32 only; a few minutes; exit 1 on any disagreement)
...
await build({ entryPoints: [entry], bundle: true, platform: 'node', format: 'esm', outfile: bundled, logLevel: 'warning' });
const { authFilePath, grokHomeFor } = await import(pathToFileURL(bundled).href);
if (typeof authFilePath !== 'function' || typeof grokHomeFor !== 'function') throw new Error('probe:home: lookup exports missing');
...
function grokFound(home, folder) {
...
const jobs = [];
cases.forEach((c, i) => { for (const placement of ['P', 'L']) jobs.push({ c, id: `${c.kind}${i}${placement}`, placement }); });
const results = [];
async function run({ c, id, placement }) {
  ...
  const grok = await grokFound(home, folder);
...
// what gets removed below; step back out first, since Windows will not remove a process's own folder.
process.chdir(root);
let next = 0;
try {
...
console.log(`${graded.length} runs graded, ${skipped} SKIPPED (session could not be placed), ${bad.length} disagreements`);
