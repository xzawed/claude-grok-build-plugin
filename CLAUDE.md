...
- **최신 릴리스 `v0.2.22`** (2026-09-06). 무엇이 왜 나갔는지는 `docs/releases/`와
...
- **이 머신 설치본은 `0.2.17`이고, 레포는 `0.2.20`을 선언한다 (세 버전 뒤)** — 최신을 쓰려면 오너가
  ...
  **머지 직후 바로 태그를 끊는다.**
- **다음 할 일 (이 레포): 없음.** 감사 두 번이 연 23건이 전부 닫혔다 — 2026-09-05 기능 감사 20건
  (FAIL 4건 v0.2.19 · A1~A6 v0.2.20 · A7~A20 v0.2.21)과 2026-09-06 **전체 감사** 3건
  (A21·A22는 v0.2.22, A23은 발견과 동시에). 전체 감사는 기능이 아니라 **"광고한 계약이 실제로
  지켜지는가"**를 물었고, 그래서 9개 중 8개 도구가 `additionalProperties: false`를 광고만 하던
  것을 잡았다. `docs/10-service-audit-queue.md`의 A 섹션은 비어 있고 그 문서는 **지우지 않는다**:
