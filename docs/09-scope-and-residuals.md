...
**이 플러그인 레포의 의도된 제품 범위(다리 + 협업 표면 + first-mile + 소비자 계약/키트)는 완료다 (최신 릴리스 `v0.2.19`).**  
...
**v0.2.18·v0.2.19 런 — 아직 돌지 않았다 (2026-09-05 확인).** §4 B가 이 체크리스트를
**릴리즈마다 1회**로 정의하는데 그 사이 두 번 나갔다(`gh release list` → Latest `v0.2.19`).
이 머신 설치본은 아직 `0.2.17`이다(`claude plugin list` = Version 0.2.17; 캐시에 0.2.18·0.2.19
디렉터리 없음). 오너가 위 순서로 갱신·재시작한 뒤 2~8단계를 실행하고, 결과를
`### 실행 기록 — v0.2.19` 절로 남긴다 — 두 릴리스를 한 번에 수락하는 셈이므로 v0.2.18의
만료 세션 분류와 v0.2.19의 라우터 위험 게이트를 **둘 다** 확인한다.
