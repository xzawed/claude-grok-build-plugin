...
>
> ⚠️ **그리고 이 문서는 *최신*보다도 뒤처질 수 있다 — `probe:contract`는 그것을 재지 않는다.**
> 2026-09-23 실측: 설치 스크립트가 *"Fetching latest stable version… Installing Grok **1.0.41**"*
> 이라 말하는 동안 개발 머신도 스냅샷도 **1.0.30**이었고 probe는 `drifted: false`였다. probe의
> 질문은 "이 머신의 CLI가 스냅샷에서 벗어났는가"이고 그 답은 옳다 — **"새 사용자가 받는 것과
> 계약이 맞는가"는 다른 질문이고, 지금 아무도 재고 있지 않다.** 왜 1.0.41이 아니라 1.0.30에
> 머물렀는지(플랫폼 차이인지, 단순히 갱신이 안 된 것인지)는 **미규명**이다 — 단정하지 말 것.
