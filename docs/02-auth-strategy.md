...
- **grok home은 `~/.grok`이 기본일 뿐 고정이 아니다.** `GROK_HOME`이 설정돼 있으면 그쪽으로
  ...
  **상대 경로 `GROK_HOME`은 grok의 작업 폴더(`--cwd`가 있으면 그 폴더) 기준으로 풀리고 `~`는 풀리지 않는다**(win32 실측;
  Windows에서는 드라이브 없는 `\x`도 그 폴더의 드라이브를 따른다) —
...
4. **인증 상태 확인은 모드별로 분기한다** (`grok_auth_check` / `checkAuth`).
   ...
   - 구독 모드: **`authFilePath(env, 폴더)`**(=`GROK_HOME`||`~/.grok` 아래 `auth.json`; 상대 `GROK_HOME`은
     ...
     `--cwd`가 있는 `grok_cli`, `worktree: true` 위임)은 추측하지 않고 통과시킨다. 위임은 서버의 사전 확인이
     다시 본다. **`grok_cli`에는 서버 확인이 없으므로** 그때 남는 것은 grok 자신의 확인뿐이다 — 상대 경로에서
     프롬프트를 실은 `grok_cli`는 `cwd`를 넘기고 인자에 `--cwd`를 넣지 않아야 hook이 확인한다.
