...
- **최신 릴리스 `v0.2.24`** (2026-09-12). 무엇이 왜 나갔는지는 `docs/releases/`와
...
- **설치본 갱신은 순서가 있다 — 클론이 먼저다.** 마켓플레이스 클론은 `autoUpdate: false`라
  ...
  **머지 직후 바로 태그를 끊는다.**
- **다음 할 일 (이 레포): 없음.** 감사 세 번이 연 25건이 전부 닫혔다 — 2026-09-05 기능 감사 20건
  (FAIL 4건 v0.2.19 · A1~A6 v0.2.20 · A7~A20 v0.2.21), 2026-09-06 **전체 감사** 3건
  (A21·A22는 v0.2.22, A23은 발견과 동시에), 같은 날 **릴리스+전체 코드 감사** 2건(A24·A25는
  v0.2.23). 감사마다 질문이 달랐다는 게 요점이다 — 기능이 도는가 → 광고한 계약이 지켜지는가 →
  **광고하지 않은 경로로도 계약이 지켜지는가**. 마지막 질문이 잡은 게 A24다(값을 붙여 쓴 `-p`가
  인증 게이트와 이력 양쪽에서 동시에 사라졌다).
  `docs/10-service-audit-queue.md`의 A 섹션은 비어 있고 그 문서는 **지우지 않는다**:
  재현 하네스와 B(측정 불가)·C(손대지 말 것)가 다음 감사에 필요하다. 새 결함은 거기 A 섹션에
  적는다 (**번호는 재사용하지 않는다** — A27까지 썼으므로 다음은 A28이다).
  범위 밖(외부/수동/보류)은 `docs/09`이고, 기각·반증된 항목을 다시 제기하기 전에는
  `docs/09`·`docs/releases/`의 근거부터 읽을 것.
- **`v0.2.24` 수락 — 한 칸 빼고 끝났다. 사람이 할 일은 없다.** 머지 → 즉시 태그·릴리스(Latest) →
  클론·설치본 갱신(`plugin list` 0.2.24 enabled) → 캐시 번들 = 커밋 blob(개행 정규화 후 동일) →
  `accept-release.mjs` **10/10** → **두 결함의 재현 페이로드를 캐시 번들에 다시 쳐서** 확인
  (A27 적대적 6형태 0 INCONSISTENT, A26 240초 정지 → 296ms+진단) → 실제 349행 이력이 정합하고
  `unknown` 전부 0(정상 데이터 무영향). 실행 기록은 `docs/09` §5.
  **남은 한 칸:** 갱신 뒤 **새로 시작된 세션**이 `serverVersion: 0.2.24`를 말하는지. 다음 세션이
  `grok_build_status`를 한 번 부르면 끝이고, 사람을 기다릴 필요는 없다.
  ⚠️ **다음 릴리스도 마지막 한 칸은 다음 세션 몫이다** — 세션은 **자기가 시작할 때의 MCP 프로세스를
  물고 있어서**, 갱신을 수행한 세션은 갱신 뒤에도 옛 번호를 말한다(`0.2.22`↔`0.2.23`에 이어
  v0.2.24에서도 재실측 — 설치본이 0.2.24인데 갱신한 세션은 `0.2.23`이라 답했다). 증거는 새 세션의
  `grok_build_status`뿐이다 (= `/grok:status`의 구동부, `commands/status.md`).
