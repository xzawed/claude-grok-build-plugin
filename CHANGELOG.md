...
상세: `docs/releases/v0.2.35.md`. grok 쪽 실측: `docs/specs/grok-cli-contract.md` §8.

- **어떻게 나왔나:** 오너가 "후속작업을 Grok과 함께"라며 A36 착수를 맡겼다. 배포 v0.2.34 번들로 재현했다 —
  같은 합성 홈이 끝 공백 없이는 `ok: true`, 끝 공백이면 `not_logged_in`.
- **전제가 틀렸다.** A36은 "grok은 끝 공백을 버린다"(`grok du --json`)에 기대 "로그인해 있어도 거절한다"고 적었다.
  오라클을 둘 더 댔다. 합성 세션을 둔 `grok models`, 그리고 위임 모양의 헤드리스 실행("Not signed in" 대 401)이다.
  둘은 함께 잰 모든 칸에서 일치했고, 끝 공백이면 **grok도 로그인하지 않은 상태**였다. 플러그인의 판정이 맞았고,
  틀린 것은 "`grok login`하라"는 안내였다. `du`만 다듬은 홈을 보고한다.
- **진짜 불일치는 적힌 증상 밖에 있었다.** Node fs는 드라이브·UNC 경로를 `\\?\`로 바꿔 Windows 정규화를 건너뛰고,
  grok의 조회는 거친다. 이름이 점·공백으로 끝나는 실제 폴더로 규칙을 좁혔다: **R1** 뒤에 성분이 오는 성분은 끝 점이
  정확히 하나일 때 잃는다(`h.`→`h`, `h..`는 그대로), **R2** 작업 폴더는 마지막 성분 끝의 공백·점을 잃는다(구분자로
  끝나면 아니다). 적어 둔 최소 수정 후보("grok과 같은 정규화")를 `du`처럼 다듬는 것으로 읽어 그대로 했다면 끝
  공백에서 거짓 "준비됨"을 만들었을 것이다.
- **무엇:** `env.ts` `grokHome`·`grokHomeFor`가 Windows에서 R1·R2를 적용한 이름을 돌려준다(바꿀 것이 없으면 같은
  문자열, `\\?\`·POSIX·기본 홈은 그대로). `grokHomeNote`는 `GROK_HOME` 앞뒤 공백의 종류·위치를 말하고 재시작까지
  안내한다(공백뿐이면 변수를 지우라고) — status·auth check 필드, 거절 메시지, hook deny(홈이 폴더와 무관하면 폴더를
  모르는 호출에서도). status의 `nextSteps`는 세션이 없어서 준비가 안 됐을 때(`not_logged_in`) 메모를 먼저 가리킨다.
- **차등 비교 (grok 자신이 독립 구현, 쿼터 0):** 손으로 고른 81가지 v0.2.34 불일치 24(거짓 거절 23·거짓 "준비됨" 1)
  → **0**. 생성한 357가지 × 세션 위치 2곳(714회): v0.2.34 불일치 456(두 방향 228씩) → **0**. 생성 쪽은
  `npm run probe:home`으로 남겼다(실제 폴더 7가지를 더해 728회 — 드라이브 문제를 고친 판으로 이 소스에서 건너뜀 0, 불일치 0).
- **Grok과 함께 — 프롬프트 형태별 결과 (이 작업):**
  - 사실 35행 **분류**(기본 effort): 300초 캡 → `resume` 240초 캡. 후보 **제안**(기본 effort): 300초 캡 → `resume`
    240초 캡. 두 세션의 로그에는 추론 스트림이 남지 않아 회수할 것이 없었다.
  - 행 수를 줄이고 **`effort: low`**로 준 제안(8행)과 메시지 판정(6행)은 각각 3턴에 끝났다 — 둘을 함께 바꿨으니
    어느 쪽 덕인지는 가르지 않았다. 제안 8가지는 모두 v0.2.34의 진짜 불일치였다(표에 넣었다). 메시지 판정은 진짜
    지적 넷을 냈다 — 재시작 안내 누락, 탭을 "공백 문자"로만 부름, 앞 공백에 cmd `&&` 설명, 끝의 탭·CR·LF나 드라이브
    경로 앞의 공백이면 grok은 세션을 놓치는 게 아니라 실패함(그렇게 잰 것은 이 경우들뿐이다). 전부 고쳤다. A35 메모 문장에
    대한 1건은 준 사실 밖이라 기각했다(계약 §8에 측정).
  - 수정본 **반례 찾기** 6행(`effort: low`): 300초 캡 → `resume`의 답은 "끝낸 후보가 없다". 통과로 세지 않았고,
    반례는 위 생성 차등 비교로 찾았다.
- **수락:** `accept-release.mjs`에 `A36` 칸(spawn 없음; 점 반쪽은 win32에서만 돌고, 안 돈 반쪽은 보고하지 않는다).
  레포 14/14, 설치된 0.2.34는 이 칸만 실패(13/14).
- **머지 전 검토 (독립 검토자 셋 + Grok 메시지 판정) — 두 번째 커밋.** 코드: 막을 결함 없음(GetFullPathNameW를 두 번째
  오라클로 약 13만 가지, 회귀 0). 빈틈 넷 — 폴더 없는 hook 메모(되돌려도 녹색이었다), 세션이 이름 끝에 공백이 있는 폴더에
  실제로 있을 때 "고치라"던 메모(통과한 사용자를 옮길 뻔), `\\?\` 작업 폴더 분기, 메모의 앞/끝 표현 — 테스트를 넣고 각각
  되돌린 코드에서 실패하는 것을 봤다. 반례: `\??\` 접두사 하나만 달랐다(v0.2.34부터, Node fs가 조회 불가, 거짓 거절 쪽 —
  알려진 한계로 적었다). 문서: 약 20곳(커밋 안 된 하네스를 가리킨 계약 문장 → `probe:home` 커밋, 한 방향이라 쓴 불일치,
  "대신"/"먼저" 혼재, hook 메모 범위, 재지 않은 `inspect`, 메모와 반대로 로그인만 안내하던 `nextSteps` 등).
- **두 번째 커밋 재검토 — 세 번째 커밋.** 새 메모 코드에는 결함이 없었다(15가지 값을 win32·linux에서 돌려 모든 문장이 참).
  대신 **그 커밋이 넣은 `nextSteps` 단계가 결함이었다** — "grok 미설치"·api 모드 "키 없음"에도 메모를 먼저 가리켰다(서버
  거절은 `not_logged_in`에만 메모를 붙인다) → 같은 규칙으로 게이트. 되돌려도 녹색이던 곳 셋(탭뿐인 값의 cmd 설명, 공백뿐인
  값의 종류 이름, 통과한 경우의 문장)에 테스트를 넣고 각각 되돌린 코드에서 실패하는 것을 봤다. `probe:home`: 설정까지
  `finally` 안으로, 채점 0이면 exit 2, 삭제 재시도, 임시 폴더 경로 자체에 R1이 바꿀 성분이 있으면 돌지 않음. 문서: 잰 범위를
  넘은 "앞 공백이면 실패", 건너뜀 수 없는 728회.
- **세 번째 커밋 재검토 — 네 번째 커밋.** 제품 코드 결함 없음(돌연변이 7개 전부 잡힘; `status`는 인증 모양 7가지 × 메모
  유무를 두 커밋으로 비교). 결함은 `probe:home`의 실패 경로에 있었다 — `EBUSY`는 재시도되지 않았고, `finally`가 던져
  결과를 가렸고, 준비 실패가 exit 1이었고, 임시 폴더 검사는 발동할 수 없었다(`mkdtemp`가 먼저 정규화한다). probe를
  다시 짰다(아래 하네스 7). 문서 셋(`nextSteps` 범위, 끝의 CR·LF도 잰 것, 재시도 서술).
- **네 번째 커밋 재검토 — 다섯 번째 커밋.** 다시 짠 probe에도 빈틈이 남았다: 준비 단계 셋(`mkdtemp`, esbuild 불러오기,
  번들 불러오기)이 여전히 exit 1(= 불일치)로 죽었고, TEMP 검사는 `/` 표기를 놓쳐 실제 폴더가 조용히 남았으며, 멀쩡한
  `x..`·끝 공백 폴더는 거절했다. 각각 exit 2로, 검사는 `win32.resolve` 뒤 점 **하나**로 끝나는 이름만 보도록(R1과 같은
  규칙) 고쳤고, Ctrl+C에도 임시 폴더를 지우며, 끝의 버전 출력이 실패해도 결과를 찍는다. 제품 코드는 바뀌지 않았다.
- **다섯 번째 커밋 재검토 — 여섯 번째 커밋.** 그 커밋의 `short()`가 esbuild 오류의 이유(둘째 줄)를 잘라 "Build failed with
  1 error:"만 남겼고, 문자열로 바꿀 수 없는 값에서는 스스로 던져 exit 1이 됐다 → esbuild는 첫 오류의 위치·문장을 말하고
  `short()`는 던지지 않는다. 상대 경로 TEMP에서는 모든 배치가 "밖"으로 세어졌다 → 임시 폴더 경로를 풀어서 쓴다. TEMP
  검사(149가지 중 147 정확, 나머지 2는 거절하는 안전 쪽)와 Ctrl+C 정리(실제 콘솔 이벤트)는 맞았다. 제품 코드는 바뀌지 않았다.
- **하네스가 틀린 것 (다음 세션이 같은 함정을 밟지 않게):**
  1. **A36 자체** — 오라클 하나(`du`)로 세운 결함이었다. grok이 *어디를 본다*고 말하는 명령과 grok이 *실제로 여는*
     곳은 다를 수 있다. 세션을 여는 경로는 세션을 여는 명령(`models`, 헤드리스 실행)으로 잰다.
  2. 재현 폴더의 `auth.json`이 `{}` 자리표시자라 grok은 경로와 무관하게 "not authenticated"였다 — 존재만 보는
     플러그인과 비교하려면 합성 세션(`scripts/synthetic-auth.mjs`)을 둬야 한다.
  3. 생성 차등 비교의 첫 판은 표본의 **절반을 조용히 건너뛰었다.** 위치를 정하는 함수를 그것을 내보내지 않는
     번들에서 가져와 호출마다 예외 → "건너뜀"으로 셌고, 요약은 반쪽 위에서 "불일치 0"이었다. 두 번째 판은 필요한
     함수가 없으면 멈춘다.
  4. 수정 중간 판이 작업 폴더 끝을 `/[ .]+$/`로 다듬었다 — IDE 진단이 짚었고 V8에서 재 보니 이차 시간이었다
     (공백 4만 개에 901 ms, 두 배마다 4배). 반복문으로 바꾸고 20만 개 입력 테스트로 고정했다(정규식으로 되돌리면
     22.5초 걸려 테스트가 실패하는 것을 확인했다).
  5. 번들 e2e에 넣은 A36 블록이 서버 호출로 끝나자 `afterAll`의 폴더 삭제가 `EBUSY`로 두 번 실패했다. A36 블록이
     없는 원래 파일로는 3/3 나지 않았다. 추정 원인은 죽인 자식 프로세스가 아직 그 폴더(작업 폴더)를 쥐고 있던 것이다 —
     헬퍼가 자식의 종료를 기다린 뒤 돌아오게 하자 3/3 통과했고, 검토자는 12회 연속 통과를 봤다.
  6. 레포에 넣은 `probe:home`의 첫 실행이 **다른 드라이브에 폴더를 만들었다.** 드라이브 없는 루트 표기의 세션을
     Node가 이 프로세스의 드라이브(D:)에 풀어 `D:\Users\…`가 생겼다(생성 시각과 내용으로 그 실행의 것임을 확인하고
     지웠다). 그 표기는 이제 케이스 폴더의 드라이브에 두고, 모든 배치가 임시 폴더 안인지 확인한다(밖이면 건너뛰고 센다).
  7. 그렇게 고친 판은 **자기 임시 폴더를 지우지 못했다**(`EBUSY`). 임시 폴더로 옮겨 간 뒤에 esbuild를 불러, 계속 살아
     있는 esbuild 서비스 프로세스가 그 폴더를 작업 폴더로 쥐었다. `rmSync`의 `maxRetries`는 `EBUSY`를 재시도하지 않아
     곧바로 던졌다 — "재시도가 버틴다"던 주석도 틀렸다(재검토가 Node 22.18로 확인). `esbuild.stop()`도 프로세스가 죽기를
     기다리지 않는다. 그래서 probe는 이제 번들을 메모리에서 만들어 `data:` URL로 가져오고(디스크에 빌드하지 않는다),
     작업 폴더를 옮기지 않으며, `EBUSY`는 스스로 기다려 다시 지우고, 끝내 못 지우면 경로를 알린다. 남은 폴더는 지웠다.
...
- **Grok 프롬프트 형태별 집계 (2026-09-24, v0.2.33~v0.2.34 작업 전체 — `CLAUDE.md` §5 규칙의 근거):**
  - 코드를 **추적**하게 한 새 프롬프트(반례 만들기 포함) 5/5가 300~360초 캡에 걸렸다. 후보만 **제안**하게 한 것도
    1/1 걸렸다(후보는 스트림에서 회수).
  - 사실을 주고 **분류**하게 한 새 프롬프트는 5번이다. 첫 실행에 끝난 것 2번(A35를 찾은 G2, hook 판정), 캡에 걸린
    세션을 `resume`해 "이미 한 분석으로 지금 쓰라"고 하자 끝난 것 2번(서버 판정 — worktree 가상 폴더 결함을 찾았다 —
    과 그 재판정), 캡에 걸려 스트림에서 회수한 것 1번(v0.2.33의 읽기 함수 10상태)이다.
- **수락:** `accept-release.mjs`에 `A35` 칸을 넣었다(spawn 없음). 설치된 0.2.33은 이 칸만 실패한다(12/13).
- **머지 전 검토 (독립 검토자 셋: 코드·테스트 / 문서 / 반례) — 두 번째 커밋.** 지적은 전부 다른 방법으로 재확인했다.
  - 코드: hook이 **읽을 수 없는 페이로드**를 상대 경로에서 통과시켰다(main은 거부) → 폴더를 모를 때의 양보는 읽은
    페이로드에만. **배선 무보호** — 검토자가 스크래치 사본에서 `defaultServerDeps`의 폴더 전달과 status 필드를 빼도
    대상 유닛 스위트가 전부 녹색이었다 → 배포 번들 e2e(`relative-home-e2e.test.ts`) 추가. 같은 두 가지를 빼고 빌드하자 그
    테스트만 2개 실패했다(유닛은 녹색). 거절 메모가 "grok 미설치"·"키 없음"에도 붙었다 → `not_logged_in`에만.
  - 반례: Windows의 **드라이브 없는 루트 경로**(`\x\gh`)를 Node가 절대 경로로 봐 묻는 프로세스의 드라이브에 뒀다.
    grok은 작업 폴더의 드라이브에 둔다 — `grok du`로 재현(C: → `C:\…`, D: → `D:\…`, `--cwd <D:>` → `D:\…`).
    → `grokHomeDependsOnFolder`. 끝 공백은 grok이 버리고 플러그인은 버리지 않아 로그인 상태를 `not_logged_in`으로
    보고했다 — A35 전부터 있던 것이라 **A36으로 열었다**(배포 번들 `grok_auth_check`로 재현). [정정 2026-09-25: 공백을
    버리는 것은 `grok du`의 보고뿐이다 — 세션 조회는 버리지 않아 grok도 로그인 안 된 상태였다. v0.2.35 항목.] `--cw` 약어는 grok이
    거부해(exit 2) hook의 `--cwd` 판별로 충분하다. 닿지 않는 UNC `cwd`의 약 21초 대기와 없는 `cwd`의 "로그인 필요"
    안내는 알려진 한계로 적었다.
  - 문서: `grok_cli`에 서버 확인이 있는 것처럼 쓴 곳 4곳, `cwd`를 안 넘기던 템플릿 3곳(테스트로 고정 — 그 테스트는
    첫 실행에 `setup.md`에서 실패했다), A7 이후 낡은 "미설정이면 서버가 막는다" 3곳, 옛 스펙의 "`GROK_HOME`이 유일한
    스위치", `CLAUDE.md` §5의 집계(출처와 안 맞아 규칙만 남기고 여기로 옮겼다).
- **두 번째 커밋 재검토 (코드 / 문서) — 세 번째 커밋. 수정이 또 결함을 들여왔다.** 드라이브 없는 경로를 가리려고 넣은
  첫 판정식이 `win32.parse().root`가 구분자로 끝나기를 요구해, **끝 구분자 없는 공유 루트**(`\\srv\share`)와
  장치 경로를 "폴더에 따라 달라짐"으로 봤다. 그래서 main이 거부하던 `grok_cli` 프롬프트 실행(`cwd` 없음)이 통과했고,
  메모는 절대 경로를 "상대 경로"라고 불렀다. 검토자가 세 번들로 보였고 `path.win32`로 재현했다 → 판정을 "절대가 아니거나,
  구분자 **하나**로 시작"으로 바꿨다(둘이면 UNC·장치). 새 번들에서 같은 입력이 다시 거부되고 메모가 없다.
  문서 쪽: 없는 `cwd`의 "로그인 필요"를 한계로 적었다고 했으나 적혀 있지 않았다 → 릴리스 노트·`docs/04` 두 곳에 적었다.
  `CLAUDE.md`의 결함 건수(원천은 `docs/10`)와 날짜 없는 규칙을 고쳤고, 드라이브 없는 경로를 메모·README·`docs/04`에
  반영했다(`/x`도 `grok du`로 재어 `\x`와 같음을 확인). e2e 테스트에 30초 제한을 걸었다(기본 5초가 자체 15초 경비보다 먼저 울린다).
- **세 번째 커밋 재검토 — 코드 결함 없음, 문서 4건 → 네 번째 커밋.** 검토자가 세 번들(main·두 번째·세 번째)을 입력
  21개 × 호출 모양 3개로 돌려 새 통과가 없음을 보였다. UNC 규칙을 grok으로 쟀고(공유 경로는 그대로, 서버만 쓴 형태는
  grok이 거부) 나도 다시 쟀다 → 계약 §8. "드라이브 없는 경로"라는 느슨한 표현(UNC도 드라이브 문자가 없다)과 메모를
  "상대"로만 부르던 곳을 `\grok` 예로 바로잡았다.
- **하네스가 틀린 것 (다음 세션이 같은 함정을 밟지 않게):**
  ...
  4. 드라이브 루트 경로(`\x`, `/x`)로 `grok du`를 재자 grok이 그 홈을 **드라이브 루트에 만들었다** — `du`도 시작할 때
     홈을 만든다. 측정 스크립트는 자기 작업 폴더만 지웠고, `C:\a35probe-…` 등 세 폴더가 남았다가 다음 측정 때 발견돼
     지웠다. 루트 경로로 재는 스크립트는 그 경로도 정리 목록에 넣을 것.
...
- **무엇:** `config-keys.ts`가 `$GROK_HOME/config.toml`을 grok과 같은 해석으로 읽는다. 구독 모드에서 status와 위임
  ...
  어디에도 없다. `billing`을 보여주는 명령 템플릿 11개·에이전트·스킬 2개가 "알리되 멈추지 말라"고 지시한다
  (테스트로 고정).
...
- **판독기를 고칠 때 다시 돌리는 법(차등 비교):**
  ...
  2. 시드 PRNG로 문서를 조합해 만든다. 축은 다음과 같다.
     ...
     - 앞 잡음: 따옴표 든 주석, 여러 줄 문자열 속 가짜 머리, 여러 줄 배열, `[other]`, `[[servers]]`,
       그리고 머지 전 검토 뒤에 넣은 `[[model]]`, `[[model."m".efforts]]`, `[[a]]`+`[[a.b]]`, 모델 표 뒤의 배열 표
     - CRLF, BOM
  3. `parse()` 결과의 `model.<id>.api_key|env_key`와 `modelCredentialDecls`를 비교한다. 불일치는 Python
     `tomllib`으로 동률을 가른다 — smol-toml 1.9.0은 줄 끝 역슬래시 뒤 닫는 `"""` 앞의 따옴표를 잘못 버린다
     (반례 검토가 찾았다).
  4. **결함 주입으로 하네스가 보는지부터 확인한다.**
  5. 파일 **종류**(FIFO·장치·디렉터리)는 생성기가 다루지 않는다. `test/config-keys.test.ts`에 고정돼 있고,
     POSIX 전용 테스트는 CI의 Linux 작업에서 돈다(개수는 그 파일이 원천이다 — 여기 적었다가 다음 커밋에서 낡았다).
- **Grok 반증.**
  ...
  - 머지 전 검토 뒤 새 읽기 함수에는 파일 상태 10가지를 **분류**시켰다(반례를 만들라고 하지 않았다). 이것도 캡에
    걸렸다. 스트림에서 회수해 보니 FIFO·장치·디렉터리·소켓은 열기 전에 거부, 자라는 파일은 원래 크기로 제한이었다.
    유일한 `CLAIM_SHOWN`은 "정규 파일을 가리키는 심볼릭 링크를 따라 읽는다"였다. 의도한 동작이라 기각했다 — grok
    문서가 dotfiles 저장소로 향하는 링크 `config.toml`을 지원 구성으로 적는다(쓰기도 링크를 따라감).
- **머지 전 검토 — 에이전트 셋(코드·테스트 / 문서 / 반례), 모두 실측으로 다시 확인한 뒤 고쳤다.**
  - **회귀(가장 무거움):** Linux에서 `config.toml`이 쓰는 쪽 없는 FIFO이거나 `/dev/zero` 링크이면, 동기 읽기가
    돌아오지 않았다. 이 읽기는 모든 spawn 전과 status에서 돌므로 **서버 전체가 응답을 멈췄다**(route까지). main은 이
    파일을 읽지 않으니 이 기능이 만든 회귀였다. 이제 정규 파일이고 1 MiB 이하일 때만 연다. 같은 스크립트로 같은
    조건을 재니: 수정 전 번들은 status 8초·route 3초 무응답, 수정 후는 status 70~72ms(`config_unreadable`)·route 2ms.
  - **오경보:** `[[model]]` 뒤의 `[model."x"]`는 TOML상 배열 원소의 하위 표인데 모델 `x`로 보고했다(검토자 둘이 따로
    찾음). grok 1.0.41은 그때 모델 재정의를 전부 무시한다(`inspect` 경고, 키 미사용 — 실측). 배열 표 접두 아래는
    모델 표로 보지 않는다.
  - **크기:** 모델 목록 상한 20개 + `modelsOmitted`, 판정 순서 Set, win32 소문자 색인, 문자열 조각 누적. 검토자
    측정으로 10만 모델 8.8초, 8 MB 문자열 0.79초였던 경로다.
  - **테스트 공백 5가지:** 결함을 주입하면 잡히는지 하나씩 확인했다. 대상은 `model` 루트 검사, 정확히 3단계,
    `api_key`→`env_key` 순서, 여러 줄 문자열 두 모양, spawn 전 계산 순서다. FIFO 테스트는 늦게 오는 쓰기
    프로세스를 붙여, 구현이 되돌아가도 멈추지 않고 1초에 실패한다(Linux 컨테이너에서 확인, CI 작업엔 시간 상한이 없다).
  - **문서 13건:** 보안 문구 "서버는 자격증명을 읽지 않는다"(README·SECURITY·`docs/02`)와, CAUTION의 "셸의 키는
    절대 자격증명이 될 수 없다"를 바로잡았다(`env_key`가 가리키는 변수는 예외). 첫 성공·프리셋 표면, 소비자 예시,
    `docs/01`·`03`·`04`, 스킬의 낡은 `grok_cli` 문장도 고쳤다. `observeBilling`의 정상 분기와 도구 설명도
    `billingCaveat`를 가리키게 했다.
  - **넓힌 차등 비교:** 새 판독기는 80,008건(시드 424242: 유효 78,991, 자격증명 있음 65,968)에서 불일치 0이다. 같은
    생성기에 옛 판독기를 넣으면 초과 보고 2,600건이다 — 이 부류를 이제 본다. 공식 코퍼스 throw 0, Grok 후보 전부 일치.
    (재현 시드: 첫 비교 12345, 결함 주입 확인 777, 넓힌 비교 424242.)
- **수정 커밋의 재검토 — 내 수정이 결함을 하나 만들었다.**
  - **결함:** 배열 표 접두 검사가 이차였다. 각 머리를 앞선 `[[…]]` 경로 전부와 비교했기 때문이다. 1 MB 유효 파일에
    배열 머리 9.6만 개를 넣으면 27초가 걸렸다. 고치려던 FIFO와 같은 부류의 멈춤이다. 이제 자격증명이 들어갈 수
    있는 머리(2부분 이하)만 보고, 그런 배열 경로만 Set에 두어 머리마다 조회는 둘 이하다.
  - **남아 있던 비용:** 키마다 표 경로 전체를 복사했다. 20만 부분 머리 뒤에 키 5.9만 개면 50초였다(수정 전부터
    있던 비용이고, 릴리스 노트의 "선형"을 거짓으로 만들었다). 자격증명 깊이(3)를 넘는 경로는 이제 만들지 않는다.
  - **최종 크기에서 잰 판별력(상한 1초):** 배열 6만 개는 이전 수정본 8,429ms, 새 판독기 37ms다. 깊은 머리 3만 ×
    키 3만은 5,152ms 대 17ms다. 3만 개에서는 1,969ms 대 19ms라 상한과 너무 가까워 크기를 키웠다.
  - **테스트 공백:** 기본 의존성이 안전한 읽기를 쓰는지 아무 테스트도 고정하지 않았다. 되돌려도 전부 녹색이었고,
    실제 FIFO는 멈췄다. 기본 경로를 거치는 끝단 테스트 둘을 넣었다.
    - 1 MiB를 넘는, 키가 든 정규 파일: 상한을 풀면 `config_model_keys`로 실패한다.
    - POSIX의 FIFO: 늦게 오는 쓰기 프로세스가 **키가 든 유효 TOML**을 넣는다. 그래서 순진한 읽기는
      `config_model_keys`로 실패하고 멈추지 않는다.
  - **사소:**
    - id·변수 이름을 200자에서 자른다(20개 × 긴 id로 2백만 자였다).
    - 도구 설명과 `docs/08`에 "확인 못 함"을 넣었다.
    - README의 "아무 변수나"를 "그 두 변수가 아닌"으로 고쳤다. 지워지는 두 변수는 보고하지 않기 때문이다.
    - 완료 조건 6번의 표면 목록(14곳)을 고쳤다.
    - FIFO 테스트의 쓰기 프로세스를 프로세스 그룹째 종료한다(`sleep`이 1초 고아로 남았다).
  - 재검토자 자신의 하네스도 네 번 틀렸다. 부분 export로 가짜 실패 21건, CRLF 때문에 돌연변이 치환 0건, 번들
    경로 주석 차이를 불일치로 오인, 존재하지 않는 잠금 상수를 썼다. 모두 결론 전에 스스로 잡았다.
- **마지막 수정 커밋의 재검토 — 코드 결함 없음.** smol-toml·tomllib 양쪽과의 표적 사례 51개, 구조 퍼즈 150만 건,
  돌연변이 6종으로 확인했다. 남은 것은 문서 드리프트 하나와 사소 셋이었고 모두 고쳤다.
  - 문서 드리프트: 이 항목이 적었던 "POSIX 전용 테스트 둘"은 다음 커밋에서 셋이 됐다. 이제 개수를 적지 않는다.
  - 이름 자르기가 UTF-16 단위라 이모지가 경계에 걸리면 반쪽이 남았다. 이제 통째로 뺀다. 경계 주변 7개 위치를
    `isWellFormed`로 확인했다.
  - JSDoc이 잘린 id를 "적힌 그대로"라고 했다.
  - 시간 테스트가 6만 개 인자를 펼쳐 V8 한계의 절반에 있었다. 이제 `join`으로 만든다.
  - 이 검토자의 하네스도 세 번 틀렸다. 하나는 `core.autocrlf`로 export한 CRLF 소스를 모르고 컨테이너에서 돌린
    것이다. 또 하나는 부분 export로 가짜 실패 24건을 낸 것이다. 마지막은 `node -e`로 돌연변이 사본을 만든 것으로,
    규칙 위반이지만 앵커 확인으로 결과는 유효했다.
- ⚠️ **이번에 하네스가 틀린 것 — 내 것 일곱 번, 검토자 것 두 번.**
  ...
  - ④ 내 차등 생성기의 앞 잡음 축에 `[[model]]` 접두가 없었다. 그래서 "80,008건 불일치 0"은 그 부류를 보지 못한
    0이었다. 검토자 둘이 각자 더 넓은 생성기로 찾았다.
  - ⑤ 기준 파서 자체의 결함(smol-toml, 위 재실행 절차 3). 내 생성기는 그 모양을 만들지 않아 영향이 없었지만,
    넓히면 가짜 "판독기 결함"이 된다. 반례 검토자는 첫 판에서 200건을 그렇게 얻고 tomllib로 가렸다.
  - ⑥ 측정 스크립트 안에서 설정 파일을 셸 `printf`로 만들려 했다. 실행 전에 알아채고 `writeFileSync`로 바꿨다.
  - ⑦ 옛 판독기를 다시 번들하면서 import 대상 파일을 같이 복사하지 않았다. 빈 출력은 결과가 아니라 하네스 실패였다.
  - 검토자 쪽: 코드 검토자의 첫 퍼즈는 깨진 난수 생성기로 "차이 0"을 냈다(손으로 만든 사례와 모순이라 잡았다).
    반례 검토자의 첫 번들 실행은 셸에서 역슬래시를 잃어 모든 경우가 "caveat 없음"으로 나왔다.
...
- **v0.2.32 마지막 칸:** 사람의 재시작 없이 `claude -p`로 새 세션을 띄워 닫았다. 새 claude.exe의 MCP 자식이
  `…/grok/0.2.32/…`로 떴고 `grok_build_status`가 `serverVersion=0.2.32`를 돌려줬다(`docs/09` §5).
- **B6 닫힘 — env만으로 생기는 우발적 종량제 폴백은 두 키뿐이다**(계약 §10 끝). 가짜 값으로 1.0.30·1.0.41을 쟀다.
  남는 종량제 경로는 사용자 `config.toml`의 모델별 키이고, 이미 범위 밖으로 결정된 경로다. `GROK_DISABLE_API_KEY_AUTH`는
  대책이 못 된다(세션이 있으면 설정 키를 막지 않는다 — 실측). Grok 반증은 `env_key`가 가리키는 변수를 "env가
  관여하는 경로"로 짚어 `CLAIM_SHOWN`을 냈다. 참이지만 새 결함은 아니다(docs/09 교훈 ③). Grok이 다음으로 재라고 한
  `GROK_AUTH`는 쟀다 — 문서에 없는 인라인 인증 저장소이고, 스키마를 알아내지 못해 미측정으로 남겼다.
- **이 머신 grok 1.0.30 → 1.0.41, 계약 스냅샷 갱신.** 표면 diff 0, `--strict` 녹색, `drifted`·`snapshotBehindLatest`
  모두 `false`. 재측정한 절은 §6(plan 모드) — 1.0.41도 쓰기를 막는다. `write` 도구 호출이 권한 단계에서 취소된
  것을 세션 기록으로 봤다(안 쓴 게 아니라 막힌 것).
- ⚠️ **판별자를 두 번 잘못 잡았다 — 둘 다 대조군이 잡았다.** ① "출력에 401이 있나"로 자격증명 사용을 판별했더니
  `XAI_API_KEY`(문서화된 폴백)도 "안 씀"으로 나왔다. grok은 그 키를 시작할 때 `api.x.ai/v1/models`로 검증하고
  400을 **디버그 로그에만** 남긴다. ② 서버 401의 `auth_kind`로 "어느 자격증명을 썼나"를 판별하고, 설정 키는
  합성 세션이 쓰지 않는 모델(`grok-4.7`)에만 걸었다. 정답은 계약 §10이 1.0.13에서 쓴 디버그 필드
  (`auth_type`/`has_api_key`)였다. 그리고 세션이 없을 때의 `auth_type=ApiKey`는 **아무것도 없어도 찍힌다**. 대조군이
  없었다면 `GROK_AUTH`·`GROK_AGENT_SECRET`를 "API 키로 쓰인다"고 잘못 보고했을 것이다.

### B4 닫힘 — 인증된 턴에서 Linux 샌드박스는 실제로 쓰기를 막는다

`docs/10`에서 B4의 남은 절반은 "실계정이 필요하다"로 열려 있었다. 호스트 `auth.json`을 컨테이너에 복사하지 않고
**컨테이너 전용 로그인**을 했다(별도 Docker 볼륨을 `GROK_HOME`으로, 오너가 기기 코드를 승인). 토큰 갱신이 호스트
세션을 깰 위험을 피하려는 것이다. 측정 뒤에는 로그아웃하고(`auth.json` 삭제 확인) 볼륨을 지웠다.

grok 1.0.41 + bwrap(`--privileged`), 같은 4단계 과제를 두 arm으로 돌렸다. `workspace`에서는 cwd 안 쓰기만 성공했고,
`/srv`에 쓰는 것은 **grok의 파일 도구도 셸도** `Permission denied`로 막혔다(`/tmp`는 프로파일이 허용).
통제 arm(`off`)에서는 전부 성공했다. 같은 컨테이너에서 두 가지를 더 쟀다.
- **A34 끝단:** 1.0.41에서도 워커의 중첩 호출이 `blocked / inside_grok_worker`로 거절됐다.
- **모델 목록:** 인증된 `grok models`로 보면 probe의 모델 diff가 0이다 — 미인증 측정의 "두 모델 사라짐"은 착시였다.

⚠️ 응답에 "제약이 작동했다"를 싣지 않는다는 판정은 그대로다. bwrap이 **그 호스트에서** 동작했는지를 래퍼는 모른다.

### v0.2.32 — 워커가 우리 플러그인을 다시 불러와 또 다른 Grok을 띄울 수 있었다 (A34)

상세: `docs/releases/v0.2.32.md`, 실측 전문: `docs/specs/grok-cli-contract.md` §14.

**잔여 점검에서 나왔다.** v0.2.31 수락의 마지막 칸(새 세션의 프로세스 명령줄)을 재려고
`Win32_Process`를 봤더니, 떠 있는 플러그인 MCP 여섯 개 중 하나의 부모가 claude.exe가 아니라
**grok.exe**였다. grok은 설치된 Claude Code 플러그인을 자기 것으로 로드한다 — 이 플러그인까지.
그래서 위임받은 워커마다 이 서버의 사본이 떠 있었고(MCP를 구성한 세션 전부 — 기록 937개 중 932개),
워커는 그것으로 또 다른 grok을 띄울 수 있었다. 재현하니 바깥 delegate는 `filesChanged: []`를
돌려줬고, 그 사이 중첩 런이 다른 디렉터리에 파일을 썼다. **실사용에서도 세 번 시도됐다**(grok 1.0.13) —
리뷰형 프롬프트를 `grok_build_verify`로 다시 넘기려 한 것이고, 그중 한 번은 worktree 격리 런에서 본
저장소를 지목했다. 셋 다 plan 런이라 grok의 권한 게이트가 취소했을 뿐이고, `--always-approve` 런은 MCP
호출을 실행한다(9/9).

**고친 방법:** `buildGrokEnv`가 모든 grok에 `GROK_BUILD_WORKER=1`을 붙이고, 그 env를 물려받은 사본은
9개 도구를 목록에 둔 채 전부 거절한다(`reason: inside_grok_worker`, spawn·이력 없음). grok의 끄는
스위치를 쓰지 않은 이유: 찾은 것(`GROK_CLAUDE_MCPS_ENABLED` 등, 비공개)은 `~/.claude.json` 서버만 끄고
플러그인 출처 서버는 그대로 둔다(실측). 숨기지 않고 거절하는 이유: 도구가 없으면 워커가 10턴·38만
토큰을 들여 찾았다. **닫는 것은 이 플러그인을 거치는 경로뿐이다** — 셸을 쥔 워커는 `grok`을 직접 띄울
수 있고, 그건 이 플러그인의 표면이 아니다.

**전제를 probe가 감시한다.** 수정은 "grok이 env를 MCP 자식에게 물려준다"는 grok의 동작 하나에 기대고,
테스트는 전부 표식을 손으로 넣으므로 그 동작이 바뀌어도 녹색이다(반례 검토자 지적). 그래서 실제 사본과
같은 조건 — 헤드리스 세션, 플러그인 출처 서버 — 을 **합성 인증**(첫 요청 401, 쿼터 0)으로 재서
1.0.30 win32, 1.0.41 Linux(샌드박스 포함)에서 도달을 확인했고, 그 측정을 `probe:contract`의
`workerMarker`로 넣었다(`reached: false`면 `--strict` 실패). 합성 인증 생성은 `synthetic-auth.mjs`로
옮겨 `probe-expired-session.mjs`와 공유한다.

**Grok 반증:** 후보 경로 7개(표식 미도달·이름 불일치·등록 순서·배선 누락·우회 spawn·원 핸들러 호출·
다른 등록 경로)를 분류시켜 `HOLE_CLOSED`, 목록 밖 경로 "none".

**머지 전 검토 세 명(코드·테스트 / 문서 / 반례) — 구멍은 없었고, 범위와 숫자를 바로잡았다.** 반영:
거절 문구에서 변수 이름 제거(읽는 쪽이 워커 모델이다), 주장 범위를 "이 플러그인 경로"로 좁힘, 보호는
grok이 로드한 **설치본**이 0.2.32 이상이고 재시작한 세션부터라는 조건 명시, 수락 스크립트 A34 세션의
모든 홈 격리(옛 번들 v0.2.7~0.2.11은 `GROK_HOME`을 안 봐서 실제 이력에 행을 쓸 수 있었다),
`best_of_n` 가드 테스트가 **첫 줄만** 보던 것을 전부 보게 강화(A34 프로브에서 빠져도 안 걸렸다 — 변이로 확인).

**그 반영분을 다시 적대적으로 검토했더니, 새 probe에 진짜 구멍이 있었다.** 격리 env가 API 키 둘만
지웠는데, grok README가 문서화한 **`GROK_AUTH_PROVIDER_COMMAND`**(외부 토큰 발급기)는 그대로 넘어갔다.
가짜 발급기로 직접 재현하니 grok은 합성 `auth.json`이 있는데도 세션을 열 때 발급기를 불렀고, 그 토큰으로
추론을 네 번 재시도했다 — 발급기가 진짜였다면 "거부될 세션"이 실제 턴이 된다. 같은 구멍이 기존
`probe-expired-session.mjs`에도 있었다. 이제 두 프로브 모두 `isolatedGrokEnv`로 부모의 `GROK_*`·`XAI_*`를
전부 지우고(고친 뒤 발급기 호출 **0회** 실측), probe는 세션이 받아들여지면(`sessionAccepted`) 그리고 grok이
플러그인 서버를 아예 안 띄우면(`blind`) `--strict`를 실패시킨다. `--strict` 종료는 모든 경고를 찍은 뒤로
옮겼고(드리프트와 같이 오면 A34 경고가 삼켜졌다), `best_of_n` 가드는 여러 줄로 나뉜 호출도 본다(변이로 확인).
플러그인 본체가 다른 자격 변수까지 지워야 하는지는 성격이 다른 질문이라 `docs/10` **B6**으로 열어 두었다.
⚠️ **그 구멍은 Grok 반증을 통과한 뒤에 나왔다.** Grok은 probe에 대해 `CANNOT_BILL`이라 판정했는데, 내가 준
facts가 "grok이 쓰는 자격증명은 API 키 둘뿐"이라고 닫아 두었기 때문이다. 판정은 facts 안에서 옳았고 facts가
틀렸다. 찾은 쪽은 정적 판정이 아니라 **가짜 발급기로 grok을 실제로 움직여 본** 검토자였다 — `CLAUDE.md` §5에
"닫힌 facts 금지" 한 줄을 남겼다.

**grok 쪽 결함도 하나 봤다 — 우리 것이 아니다.** grok의 stdio MCP 디코더가 8 KiB를 넘는 메시지를
바이트 8192에서 잘라 읽는다. 우리 `tools/list`가 12,625바이트라 기록 937개 중 335개(36%)에서 해석 오류가
났고 292개는 그 때문에 연결에 실패했다(1.0.13 `65s`, 1.0.30 `70s` 타임아웃) — 그게 A34를 간헐적으로
가렸고, 재현이 두 번 실패한 이유다.

⚠️ **내 숫자가 세 번 그럴듯하게 틀렸다 — 문서 검토자가 기록을 다시 세서 잡았다.** "923개 중 923개"는
937개 중 932개(5개는 MCP 구성 전 종료), "약 35%·70초"는 해석 오류(36%)와 연결 실패(31%)·65/70초를 섞은
값, 그리고 실사용 3건과 plan 취소를 1.0.30에 귀속했는데 **이 머신은 09-13에야 1.0.30이 됐다** — 셋 다
1.0.13이었다. 셋 다 명백히 이상해 보이지 않았다는 점이 `CLAUDE.md` 8(b)("그럴듯하면 더 의심한다")의 실례다.

⚠️ **하네스가 아홉 번 틀렸다 — 일곱 가지, ①과 ③은 두 번씩 (진짜 결함 1건 대비).** 전부 출력이 명백히
이상해서 잡혔다.
① Write 도구에 넘긴 U+2028의 역슬래시-u 이스케이프가 **실제 U+2028 문자로 풀려** 정규식 리터럴
안에 줄 끝이 생겼다(SyntaxError) — 셸이 아니라 **작성 도구가** 한 겹을 먹은 사례다. 역슬래시-n은
그대로 남았다. **그리고 이 항목을 CHANGELOG에 쓰다가 같은 함정에 또 걸렸다** — 이스케이프를 인용한
자리가 실제 문자로 바뀌었다. 파일에 그 표기를 쓰지 말고 이름(U+2028)으로 부를 것.
② 세션 기록에서 우리 도구 이름으로 넓게 찾았더니 **1471건** — grok 자신의 내부 네임스페이스가
`grok_build`라 모든 도구 호출에 붙어 있었다. `use_tool`의 `tool_name`으로 좁히니 4건(실사용 3 + 재현 1).
③ grep에 역슬래시가 든 패턴을 셸로 넘겨 0건이 나왔다(두 번) — 파일은 멀쩡했다.
④ 분석 스크립트를 엉뚱한 폴더 경로로 불렀다(`scratchpad/a34/` vs `scratchpad/`).
⑤ 1.0.41 모델 목록이 "두 개 사라짐"으로 나왔는데 **미인증 컨테이너**에서 잰 값이라 판정하지 않았다
(합성 인증 세션은 두 버전 모두 `grok-4.6, grok-4.5`만 말했다 — 목록은 인증 상태를 따른다).
⑥ Git Bash가 `docker run -v grok-b4-home:/g … ls /g`의 `/g`를 `G:/`로 바꿨다 — `MSYS_NO_PATHCONV=1`.
⑦ "데몬이 떴나"를 `leader|daemon|serve`로 찾았더니 `mcp-server`의 `serve`가 걸렸다 — 걸린 줄은 전부 node였다.
그리고 **통제군이 결론을 지켰다:** 재현이 두 번 실패했을 때 "워커는 우리 도구를 못 쓴다"로 닫을 뻔했다.
같은 시각 다른 세션의 워커는 46ms 만에 연결돼 있었고, 그 비교가 원인(디코더)을 가리켰다.

### v0.2.32 수락 — 거절을 끝단에서 봤다

`docs/09` §5. 설치된 0.2.32 사본이 실제 워커 세션에서 중첩 위임을 2ms 만에 거절했다(`reason:
inside_grok_worker`, 중첩 이력·파일 0). 수락 레포·캐시 11/11, 캐시 = 태그 blob. 새 세션 칸과 **재시작해야
보호가 켜진다**는 사실은 그 절에 있다.

### v0.2.31 수락 — 마지막 칸을 새 세션이 닫았다

`docs/09` §5. 세션 MCP의 명령줄 `…/grok/0.2.31/…`, 캐시 = 태그 blob, 로드 순서, `serverVersion:
0.2.31`, 수락 레포·캐시 10/10.

## 2026-09-23

### probe:contract가 두 번째 질문을 하게 됐다 — "새 사용자는 무엇을 받나"

`drifted: false`가 **맞는데도** 계약이 사용자가 받는 CLI를 설명하지 않을 수 있다. 2026-09-23 실측:
채널 포인터가 `1.0.41`인데 이 머신도 스냅샷도 `1.0.30`이라 probe는 정확히 "이상 없음"이라 답했다.

Grok에게 **probe 헤더가 재는 것보다 많이 주장하는가**를 물었고 답은 `HEADER_IS_SCOPED`였다 —
헤더는 "the grok CLI **in front of you**"라고만 말한다. 즉 **코드가 틀린 게 아니라 질문이 하나
빠져 있었다.** 그래서 `drifted`는 손대지 않고 `snapshotBehindLatest`를 따로 붙였다. 출처는
**설치 스크립트가 읽는 바로 그 포인터**(`https://x.ai/cli/<channel>`, GCS 폴백) — 릴리스 페이지나
체인지로그를 읽으면 사용자가 실제로 받는 것과 어긋날 수 있는 **두 번째 출처**가 생긴다.

**셋을 구분한다:** `true` · `false` · `null`(못 물어봤다, 사유 동반). `--strict`는 일부러
깨뜨리지 않는다 — grok은 자주 릴리스하므로 그걸로 게이트를 붉히면 `b721433`이 고친 늑대 외치는
게이트가 된다.

⚠️ **초안이 거짓 문장을 실었고, 로직이 스크립트 안에 있어 어떤 테스트도 닿지 못했다.**
스냅샷이 뒤처지면 무조건 *"nothing on this machine moved"* 를 찍었는데, **머신이 갱신된 경우엔
거짓**이고 하필 그 경우가 바로 위 drift 보고가 뜨는 경우라 두 블록이 서로 모순됐다. Grok에게 그
상황과 그 문장만 주니 `SENTENCE_FALSE`. **A33과 같은 모양을 A33 다음 커밋에서 또 했다.**
메시지 선택을 `scripts/published-version.mjs`로 떼어내 테스트가 닿게 했다 — `server.ts`가 남긴
교훈과 같다(호출 불가능한 로직은 계약을 뒤집어도 전 스위트가 녹색이다).

⚠️ **리뷰 프롬프트를 세 번 쪼개서야 답이 왔다.** 4 시나리오 × 2 판정 → 타임아웃, 4 문장 판정 →
타임아웃, **문장 하나 → 완료**. 두 번째가 걸린 이유로 의심되는 것: 파일에 명시되지 않은 도구
동작에 대한 조건문(`--update가 둘 다 해결한다`)이 들어 있어 추론이 열려 있었다.

⚠️ **그리고 이 작업 중에 CLAUDE.md를 인코딩으로 깨뜨렸다 — 셸 규칙의 새 구멍이다.** 가드가 여전히
무는지 확인하려고 PowerShell로 파일을 읽어 두고, 가짜 식별자를 붙였다가, 다시 써서 되돌렸다.
`Get-Content -Raw`(기본 인코딩)와 `Set-Content -Encoding utf8`이 달라 **한글 전체가 이중 손상**됐다.
**아무것도 조립하지 않았다** — 따옴표도 역슬래시도 개입하지 않았으므로 기존 규칙의 근거("셸이 한 겹을
먹는다")만 읽으면 안전해 보인다. `git checkout -- CLAUDE.md`로 복구하고 편집 도구로 다시 했다.
규칙에 **"왕복도 포함"** 을 붙였다. 이것으로 이 규칙은 세 번 넓혀졌고, 세 번 다 넓힌 직후 다시 당했다.

### v0.2.31 — 못 잰다고 적어둔 것을 재러 갔더니, 결함은 우리 쪽에 있었다 (A33)

상세: `docs/releases/v0.2.31.md`. **B4를 Docker로 실제로 쟀다.** 이 레포는 win32에서만 돌았고 샌드박스 커널 강제는 Linux
Landlock / macOS Seatbelt뿐이라, `docs/10` B4는 "리눅스 머신이 있어야 잰다"로 닫혀 있었다.
Docker가 그 머신이다 — 그리고 결함은 리눅스 커널이 아니라 **우리 메시지**에 있었다.

grok 1.0.41은 샌드박스를 적용할 수 없으면 **fail-closed**로 시작을 거부한다
(`Refusing to start with denied paths unprotected.`, exit 1, stdout 0바이트). 좋은 동작인데,
그게 래퍼의 "envelope 없음" 분기에 떨어져 **"Grok Build 출력을 해석할 수 없습니다"** 로
보고됐다 — 아무도 확인한 적 없는 원인이고, 서로 다른 세 가지 실패에 같은 문장이 나갔다.
`b721433`(릴리스 게이트)과 **같은 모양**이다: 멈춘 건 옳고 사유가 틀렸다.

하필 이 경로가 중요한 이유: `--sandbox`에는 **env 별칭**이 있다(`[env: GROK_SANDBOX=]`).
호출자가 아무것도 넘기지 않아도 오퍼레이터 셸의 변수 하나가 위임을 죽인다. 4-arm 실측 —
미설정→auth_error / `workspace`→시작 거부 / 파라미터→동일 / `off`→auth_error로 복귀 —
으로 원인이 그 변수임을 확정했다. 이제 메시지가 그 변수를 지목하고 `sandbox: "off"`를 안내한다.
일반 분기도 원인을 단정하지 않고 `rawStderrTail`을 가리킨다. 실측 전문은
`docs/specs/grok-cli-contract.md` §13(이 문서에서 **유일하게 리눅스에서 잰 절**).

**Grok이 이 수정 안에서 같은 결함을 찾았다.** 초판 신호가 `/^bwrap: /im`이었는데, `m`은 `^`를
**모든 줄 머리**에 붙인다 — grok이 작업 중 스스로 `bwrap ...`을 실행하고 그 stderr가 섞이면
"grok이 시작을 거부했다"고 보고된다. grok은 시작했고, 실패한 건 사용자의 명령이다. 즉
**A33이 고치려던 오류를 A33의 수정 안에 다시 심을 뻔했다.** 전체 stderr 머리로 앵커를 바꿨다 —
앞에 다른 출력이 있으면 일반 메시지로 떨어지고, 그쪽은 아무 원인도 주장하지 않는다.

⚠️ **반례를 "발명하라"는 리뷰 프롬프트는 타임아웃한다 (180초 × 2회).** 같은 질문을 **내가 쓴
후보 4개를 분류하라**로 바꾸니 한 번에 돌아왔고, 그 답이 위 오탐이다. `CLAUDE.md` §5의
"한 번에 한 가지 주장만"에 더해, **개방형 생성 과제를 주지 말 것** — 판정·추출은 돌아온다.

⚠️ **하네스가 여섯 번 틀렸다 (진짜 결함 2건 대비).** 전부 출력이 명백히 이상해서 잡았다.
① PowerShell 5.1이 `sh -c "…"` 안의 따옴표를 먹어 도구 탐지가 "not found"로 나왔다(3회).
② Git Bash가 `/etc/resolv.conf`를 `C:/Program Files/Git/etc/…`로, `origin/main:.claude/…`를
Windows 경로로 바꿔 측정 자체가 성립하지 않았다. ③ tool 인자 키를 `task`로 썼더니 **네 arm이
전부 동일한 스키마 오류** — 스키마가 안 막았으면 "차이 없음"으로 읽혔다. ④ BuildKit이
`--network host`를 빌드 스텝에 적용하지 않아 apt가 "Unable to locate package"를 냈다 — 패키지
부재로 읽히지만 **DNS 실패**다. ⑤ 자격증명 없이 재려 했더니 서버의 **spawn 전 게이트**가 네 arm을
동일하게 막았다. ⑥ 합성 `auth.json`을 한 번만 썼는데 **grok이 401 후 그것을 폐기**해서(계약 §7 C)
arm 2~4가 다시 게이트로 떨어졌다. **⑤⑥은 통제군이 없었으면 그대로 "샌드박스는 차이를 만들지
않는다"가 됐다** — Grok이 설계 검토에서 경고한 교란이 정확히 그대로 발생했다.

### 릴리스 게이트가 확인하지 못한 것을 단정했다

`check-release-tag.mjs`는 **스케줄로 도는** 게이트다. 그 catch가 에러를 바인딩하지 않고 **어떤
throw에도** "no GitHub release"를 밀어넣었다 — gh 미설치·미인증·rate limit·오프라인 전부. 즉
gh 토큰이 만료되면 **이미 존재하는 릴리스를 끊으라는 경보가 반복**된다. 늑대를 외치는 게이트는
사람들이 무시하게 되고, 그게 진짜 비용이다. A29와 같은 모양 — 멈춘 건 옳고 사유가 틀렸다.

이제 **"물어보지 못했다"와 "물어봤더니 없다"를 구분**한다.

⚠️ **첫 수정이 주 케이스를 거꾸로 분류했고, 양쪽 분기를 다 돌려서 잡았다.** `execFileSync`의
`Error.message`는 `"Command failed: gh …"` 뿐이고 gh의 실제 답("release not found")은 **stderr에**
있다. message를 보던 초판은 **진짜로 없는 릴리스를 "확인 불가"라고** 말했다 — 막으려던 실패를
그대로 뒤집은 것이다. 세 분기(부재 / gh 없음 / 정상)를 각각 실행해 확인했다.

그리고 `tagged ? '' : '?'`는 **도달 불가**였다(태그가 없으면 위에서 이미 exit). 없는 불확실성을
표시하고 있었다.

⚠️ **heredoc 역슬래시 함정을 다섯 번째로 밟아 이 스크립트를 깨뜨렸다.** `git checkout`으로 되돌리고
편집 도구로 다시 했다. `CLAUDE.md`가 이미 두 번 적어둔 규칙이다 — **소스는 편집 도구로 고친다.**

### 최소 런타임이 번들러 안에만 있었다 — 그리고 격리 프로브는 깨끗했다

배포되는 나머지를 계속 봤다.

- **`build.mjs` / `package.json` — 결함.** 제품의 **런타임 floor가 번들러의 `target: 'node18'`
  한 줄에만** 있었고, 매니페스트에는 `engines` 필드가 **아예 없었다**. 전역 규칙이 "최소 런타임의
  원천은 `engines`"라고 지정한 바로 그 자리다. Grok이 확증하면서 근처의 오인 후보도 배제했다 —
  `@types/node`는 devDependency 범위이지 런타임 버전이 아니다.
  ⚠️ 이 제품에서는 보통보다 더 중요하다: 엔드유저는 **빌드된 파일을 받아 자기 node로 실행**하고
  설치 시점에 의존성을 깔지 않으므로, 어떤 도구도 그들을 대신해 `engines`를 읽지 않는다. 즉 선언은
  사용자를 **보호하지 못하고**, 두 숫자가 조용히 갈라지지 않게 만드는 것이 실제 이득이다 —
  `plugin-surface.test.ts`가 둘의 일치를 강제한다(불일치를 주입해 발화 확인).
- **`probe-unauth-device-flow.mjs` — 결함 없음.** 판정 `False`. 우선하는 `GROK_HOME`이 spread
  **뒤에** 덮이고 `HOME`·`USERPROFILE`·win32 앱데이터까지 전부 임시 디렉터리로 간다. 이 파일이
  주석에 남긴 과거 실패("격리한다고 믿으면서 아무것도 격리하지 않았다")는 제대로 닫혔다.
  ⚠️ **이 판정 하나를 받는 데 세 번 걸렸다.** 파일 안에 실제 명령 배열이 있어 그때마다 타임아웃이
  났고, env 구성만 떼어내자 통과했다. `CLAUDE.md` §5의 *"명령 모양을 추론 대상으로 주지 말 것"*
  은 장식이 아니다 — 이 세션에서만 네 번째다.

### 배포되는 나머지 도구도 봤다 — 이웃의 약속이 침묵을 덮고 있었다

`accept-release.mjs`에서 결함이 나온 뒤 같은 디렉터리의 나머지를 이어서 봤다.

- **`mcpcall.mjs` — 비용을 말하지 않았다.** 바로 옆 `accept-release.mjs`가 *"spawns no grok
  process and spends no subscription quota"* 를 크게 약속하고 **같은 디렉터리로 함께 배포**되니,
  그 약속이 둘 다 덮는 것처럼 읽힌다. 실제로는 `call grok_build_delegate`(또는 plan·verify·
  프롬프트를 실은 `grok_cli`)가 **진짜 헤드리스 grok 턴**을 돌린다. Grok에게 두 헤더만 주고
  물었더니 주장을 인용해 확증했고, 독자가 비용 안내로 오인할 만한 유일한 줄
  (`api-mode branch can be exercised`)조차 **그게 종량제라는 말을 하지 않는다**고 짚었다.
  이제 헤더가 비용을 먼저 말하고, **무료인 호출을 열거**하며, 환경 설명도 고쳤다 —
  "GROK_BUILD_AUTH_MODE가 전달된다"가 아니라 `env: process.env`로 **부모 환경 전체**가 간다.
  (덤으로 낡은 괄호 `(0.2.17 here)`를 지웠다.)
- **`rpc-timeout.mjs` — 결함 없음.** 쓰레기 입력은 신뢰 대신 폴백, 기본값 아래로 안 내려감,
  마진이 **서버의 구조화된 timeout이 경주에서 이기게** 하고, 메시지가 하네스 캡과 서버 행을
  구분한다. F4가 틀렸던 것 전부가 제대로 닫혀 있다.
- **`probe-metered-billing.mjs` — 동작은 손대지 않고 기록만.** 이 파일은 오히려 모범이다
  (증명 가능/불가능 명시, 적대적 리뷰가 강한 표현을 기각한 사실, 출력 64토큰 캡, "재시도 말라"는
  exit 2, 키 비에코 입력, `set-cookie` 제거). 다만 기본 모델이 `grok-4.6`으로 **하드코딩**돼
  있는데, 이건 `probe:contract`를 만든 바로 그 결합 유형이다. **돈이 드는 프로브라 검증 없이
  바꾸지 않았고**, 대신 그 사실과 `XAI_PROBE_MODEL` 우회를 그 자리에 적었다.

### 감사 범위를 감사했다 — 배포되는데 한 번도 안 본 코드

앞선 라운드는 `mcp-server/src/`를 전수했다. 그런데 **CLAUDE.md가 경고하는 그 코드**는 범위 밖이었다:
마켓플레이스 소스가 `./`라 `.claude/tools/*.mjs` **457줄이 모든 사용자 캐시에 바이트 동일하게**
들어간다(설치본에서 확인). 같은 질문("선언하지 않은 일을 하는가")을 한 번도 받은 적이 없다.

**첫 대상에서 결함이 나왔다 — 파일을 가로지르는 과금 안전장치에 표시가 없었다.**

`accept-release.mjs`는 *"spawns no grok process and spends no subscription quota"* 를 약속하고,
delegate/plan/verify 프로브에 `best_of_n: 2`를 실어 그 약속을 지킨다. `best_of_n`은 **선언된 입력**
(`server.ts` strengthFields)이라 스키마를 통과해 `validateDelegateOptions`에서 멈춘다 — 배포 번들
실측으로 그 분기의 메시지가 그대로 돌아왔다.

그런데 **`delegate.ts`는 자기가 그 역할을 하는 줄 몰랐다.** 더 나쁘게도, v0.2.27에 내가 쓴 주석이
`best_of_n` 처리를 *"더 이상 없는 플래그의 잔재"* 처럼 읽히게 만들어 **지우기 쉬운 코드로 보이게**
했다. 지우면 릴리스 수락을 돌릴 때마다 **실제 과금 위임 3건**이 조용히 나간다.

두 파일은 서로를 import하지 않으므로 **링크를 테스트로 만들었다** — 그 테스트가 수락 도구에서
프로브 페이로드를 직접 읽어오므로, 저쪽을 고치면 이쪽이 실패한다. 프로브 하나에서 `best_of_n`을
빼서 가드가 실제로 발화하는지 확인했다.

#### Grok은 절반 맞았고, 측정이 어느 절반인지 갈랐다

주장을 던지니 **양쪽 다 False**라고 답했다. ① *"내 주석의 주어는 검증기가 아니라 삭제된 범위
상수다"* — **옳다. 내가 과장했다.** ② *"미지 키로 죽는 것이지 이 검증기가 아니다"* — **틀렸다.
`best_of_n`은 미지 키가 아니라 선언된 필드다.** 배포 번들 실측이 갈랐다.
...
증거다. `hook.ts`·`orchestrator.ts`·`routing.ts` 모두 Grok 판정 `False`(부정 진행 경로 없음).

- **먼저 내 기록 방식을 고쳤다.** v0.2.29에서 `docs/09`에 *"감사 현황은 `FOUND BY GROK` 마커가
  원천"* 이라 적었는데 **그게 거짓이었다** — 마커는 결함이 나왔을 때만 남으므로 이미 깨끗하게
  감사한 `auth.ts`·`hook.ts`가 미감사로 보였다. 다음 세션이 닫힌 질문을 다시 팠을 것이다.
  이제 **깨끗한 감사도 그 자리에 기록한다**(`AUDITED BY GROK <날짜>, no finding` + 던진 주장 + 판정).
- **`status.ts` 1건.** Grok의 답은 절반만 맞았고 그 절반이 진짜였다. 틀린 절반: "과거 모드 기록이
  없다" — `billing`은 `billingFor(mode)` 파생이라 `metered_api` 행이 곧 증거이고 팁이 그렇게 말한다
  (**팁은 유지**). 맞는 절반: `billingMismatch`는 **이력에 대한 사실**인데 `nextSteps`가 현재 고장으로
  말했다("과금 경로를 먼저 정리한 뒤 위임을 재개하세요"). 이미 구독으로 돌아온 사용자는 고칠 게 없다.
  **탐지는 건드리지 않았다** — `docs/10` §C의 "4×2 전부 정확"은 플래그를 잰 것이다.

## 2026-09-22

### v0.2.29 — 집계가 받은 데이터보다 많이 말했다

v0.2.28의 후속. 상세: `docs/releases/v0.2.29.md`. **셋을 봤고 하나에서만 결함이 나왔다** —
`auth.ts`·`hook.ts`는 깨끗했고, 그 사실이 `usage.ts`의 3건이 잡음이 아니라는 증거다.

- **`auth.ts` — 행동하지 않았다.** Grok이 "유효하지 않은 키에도 `ok:true`"라고 답했고 참이지만
  **은닉이 아니다**: A13 주석이 실측 경위와 *왜 검증 호출을 넣지 않았는지*(그 모드에선 네트워크
  왕복이 과금된다)를 적고, 사용자 메시지가 "유효성은 검증하지 않았습니다"라고 말한다.
  참이지만 이미 선언된 것과 숨겨진 것을 구분하지 못하면 없는 일을 만든다.
- **`hook.ts`** — Grok 판정 `False`. 부정 허용 경로 없음.
- **`usage.ts` 3건.** ① A17 수정이 *"필터 아래의 0과 전체의 0은 다른 사실"* 이라 적어놓고 헤드라인은
  "다른 경로의 이력은 그대로 있습니다"라고 **단정**했다 — 이 함수는 필터된 집계만 받으므로 알 수
  없다. **문구를 약하게 만드는 대신 진짜 값을 넘겼다**(`summarizeHistory`가 이미 쥐고 있었다):
  실측 "다른 경로에 **907건**이 있습니다", 같은 순간 비필터 total도 907. ② "모든 위임이 종량제"가
  metered=전체를 요구하지 않았다. ③ "구독 워커를 잘 쓰고 있습니다"가 구독 건수를 **아예 읽지 않았다**.
  ②③ 둘 다 `byBilling.unknown` 때문에 도달 가능하고, **같은 함수 두 줄 아래**가 "손으로 편집된 항목"을
  이유로 `filesCount`를 강제 변환한다 — 한쪽은 방어하고 한쪽은 안 했다.


### v0.2.28 — remove가 지우라고 하지 않은 브랜치를 지웠다

v0.2.27 감사의 후속. 그때는 `env.ts`·`server.ts`만 Grok에게 보여줬고 **되돌릴 수 없는 삭제를 하는
`worktree.ts`는 보지 않았다.** 상세: `docs/releases/v0.2.28.md`.

호출자는 worktree 하나를 이름으로 주는데, `removeGrokWorktree`는 그것을 지운 뒤 `grok/<basename>`을
유도해 **인자로 선택된 적 없는 브랜치**를 지웠다. 왜 문제인지는 이 파일이 이미 적어놨다 —
`isWrapperWorktreeName`의 주석이 *"감사에서 baseDir 안에 남의 체크아웃이 있었다; git이 못 알아본다고
우리 것이 되지는 않는다"* 라고 말하고, **prune은 그에 따라 소유권을 두 번 확인한다.** remove는 같은
판단을 디렉터리에만 적용하고 브랜치에는 적용하지 않았다 — 비대칭이 논거다.

⚠️ **가드를 넣자 기존 테스트 3개가 깨졌고, 그게 두 번째 발견이었다.** 픽스처가 `grok-abc123`·
`grok-a`·`grok-b`였는데 `worktreeName()`은 그런 이름을 만들 수 없다(`grok-<base36 ms>-<랜덤>`).
즉 그 테스트들은 **한 번도 일어날 수 없는 입력**으로 브랜치 정리를 검증하고 있었다. 레거시 위험은
`git log -L`로 확인했다 — 네이밍은 PR #2 이래 바뀐 적이 없어 다른 형식의 worktree 집단은 없다.

교훈 하나: **감사의 완결성은 무엇을 봤는가가 아니라 무엇을 아직 안 봤는가로 판단해야 한다.**


### v0.2.27 — 감사: 은닉성 코드 · 고아 문서 · 불필요한 내용

오너 목표: **현재 코드·문서를 Grok과 함께 감사**해 있을 이유 없는 것과 하고 있는데 말하지 않는 것을
찾는다. 상세는 `docs/releases/v0.2.27.md`가 원천이다. 결과는 대칭적이었다 — **지울 것은 거의 없었고,
말하지 않던 것이 있었다.**

- **Grok이 찾음:** `buildGrokEnv`의 API 키 삭제가 이 모듈에서 가장 결과가 큰 동작인데 **유일하게
  주석이 없었다**(여덟 줄 아래 보조적인 `HOME` 폴백에는 8줄짜리 근거가 있다). 근거는 `CLAUDE.md`에만
  있었고 그 파일은 **엔드유저에게 전달되지 않는다.** 동작은 테스트 9개가 고정하므로 구멍은 아니고,
  고친 것은 읽는 자리의 부재다.
- **Grok이 찾음:** `delegate`·`plan`·`verify`가 실행마다 이력을 기록하는데 **description이 말하지
  않았다.** README는 고지하지만(그 축은 결함 없음 — 확인했다) **호출자가 읽는 것은 description뿐**이고
  `docs/07`이 다루는 외부 오케스트레이터가 바로 그 독자다.
- **지움:** `BEST_OF_N_MIN`·`BEST_OF_N_MAX`. 레포 전체에 선언 2줄이 전부였고 **배포 번들엔 0건**.
- **고아 문서:** 후보 70건 중 진짜는 **1건**(`grok-streaming-json-sample.jsonl`). 나머지는 디렉터리
  규약 로드(`commands/`·`skills/`·`agents/`)이거나 gitignore된 로컬 스크래치였다. 그 1건도 지우지 않고
  계약 §2에서 인용했다 — 줄인 예시를 검증할 유일한 근거다.
- **검증되지 않던 숫자:** 라이브 문서 4곳의 "9 tools"를 `server.registerTool` 실제 개수와 대조하게 했다.

#### 내가 만든 회귀를 감사가 잡았다

v0.2.26 범프 때 python `json.dumps`가 non-ASCII를 이스케이프해 `plugin.json`의 한글이 `에…`로
커밋됐다. 파싱 결과가 같아 테스트는 통과했고 사람이 읽는 파일만 나빠졌다. **소스를 문자열 도구로
고치지 말 것** — 이번엔 JSON 직렬화기가 같은 함정이었다.

#### 하네스가 2번 틀렸다

링크 검사기가 **dangling 194건**을 냈는데 전부 오탐이었다(루트 기준 경로를 문서 디렉터리 기준으로
풀었다). 고치니 5건, 그 5건도 과거 계획서라 **실제 dangling은 0건**이다. 그리고 heredoc 역슬래시
함정을 **세 번 더** 밟았다 — 이 세션에서만 다섯 번째다.
...
오너 목표: **릴리스 서비스·전체 코드·문서·연결점**에 대한 정밀 감사. 네 항목군을 훑고, findings마다
**두 가지 독립 방법**으로 재도출한 뒤 **재검증 2회**(작업트리 / 새 클론)를 돌렸다. 무엇을 왜 고쳤는지는
`docs/releases/v0.2.25.md`가 원천이다 — 여기 옮겨 적지 말 것. 요약만:

- **F5**(제품 코드) `isPathInsideBase`의 공동 try/catch — `realpathSync`가 한쪽만 throw해도 **양쪽이**
  문자열 비교로 강등돼, 심볼릭 링크 조상이 해석되지 않았다. 적대적 리뷰가 정적분석으로 찾았다.
- **F4**(감사 하네스) `mcpcall.mjs`가 `timeout_ms`를 무시하고 240초에 끊었다.
- **F3·F2·F1**(문서) `.claude/`를 "배포 안 됨"이라 적었고(실제로는 캐시에 바이트 동일하게 복사된다),
  항상 로드되는 파일의 감사 누계와 큐 건수가 낡아 **자기 자신과 모순**됐다.

**전부 배포 번들·설치 캐시에서 재현한 뒤 고쳤고, 같은 페이로드로 다시 쟀다.** F4는 라이브로:
같은 호출이 수정 전 **정확히 240초**에 죽었고 수정 후 **606초에 `completed`** 로 끝났다.

#### 이 감사에서 하네스가 5번 틀렸다 — 진짜 결함과 같은 수다

① 백틱 경로 스윕 초판이 오탐 52건(gitignore 부정 패턴·ESM 임포트·패키지 스펙·외부 레포·
*"이 파일은 없다"고 명시한 문장* 3곳) ② 커맨드 표 파서가 중복·허위 누락 ③ 새 클론 대비 +1 차이
(원인: `.gitignore`된 로컬 디렉터리) ④ **Grok에게 준 코드 발췌가 `return` 직전에서 잘려**
"함수가 값을 반환하지 않는다"는 오판을 유발했다(Grok이 "잘린 것 같다"고 먼저 말했다) ⑤ 셸에서
문자열 이스케이프로 `version.ts`를 편집해 파일을 깨뜨렸다 — **CLAUDE.md가 이미 경고한 그 함정**.

교훈 둘: **발췌해서 리뷰시킬 때는 끝 경계를 먼저 확인한다.** 그리고 **셸 문자열로 소스를 고치지
않는다** — 편집 도구를 쓴다.

### B3 — 콘솔은 브라우저 자동화로 읽을 수 없다. 우회는 하지 않는다

`console.x.ai`를 에이전트가 직접 읽어 B3의 사전 확인(콘솔이 어떤 항목을 보여주는가)을 끝내려 했다.
**막혔다.** MCP가 띄우는 브라우저(Puppeteer/Playwright 방식 — 별도 프로필, 자동화 플래그)는
Cloudflare **하드 블록**을 받는다("Sorry, you have been blocked", Ray `a3a4764f2da03290`). 첫 페이지는
열렸고, 두 번째 이동에서 막힌 뒤에는 첫 페이지도 막혔다.

**IP 차단이 아니다** — 같은 머신에서 `api.x.ai`는 그대로 400 인증 거부를 돌려준다. 브라우저 지문
차단이므로 **Playwright도 같은 벽**이다(같은 표식을 더 뚜렷하게 단다). **우회는 시도하지 않았고 범위
밖이다** — 지문 위장도, 챌린지 해결도 하지 않는다.

남은 경로는 하나이고 **아직 실행되지 않았다**: 오너가 자기 Chrome에서 원격 디버깅을 켜고 **직접**
페이지를 연 뒤, 에이전트가 **순수 CDP로 텍스트만** 읽는다. MCP 브라우저 서버를 쓰지 않는 이유는
적대적 리뷰가 짚은 실패 조건 때문이다 — **attach 경로가 Puppeteer/Playwright init 스크립트를 주입할
수 있다.** 같은 리뷰가 이 방식을 "봇 차단 우회와 성격이 다르다"(위장 없음, 오너의 세션, 사람이 연
페이지를 읽기만)로 판정하면서, 막히면 **중단**이 정답임도 함께 적었다.

부수로 확인된 것: 공개 문서는 콘솔에 **Usage explorer**가 있다고만 하고 그 맥락은 **선불 크레딧·API
소비**다. **CLI·구독 사용이 거기 보이는지는 문서가 말하지 않는다** — B3가 이 계측기로 닫히지 않는
이유가 문서 수준에서도 확인된 셈이다.

## 2026-09-12

### B3 계측기 프로브를 레포에 남겼다 (`npm run probe:metered`) — 버전은 올리지 않는다

종량제 콘솔의 0은 아무것도 닫지 못한다(집계 지연·안 뜨는 SKU·다른 키). 그래서 **계측기를 먼저
검증하는** 프로브를 `mcp-server/scripts/probe-metered-billing.mjs`로 남겼다: 의도적으로 과금되는
요청을 딱 한 번(`max_output_tokens: 64`) 보내고, 콘솔에서 **요청 수**로 찾게 한다. 스크래치패드에
두면 세션과 함께 사라지므로 기존 `probe:unauth`·`probe:expired` 옆에 둔다.

**적대적 리뷰가 1라운드에서 다섯 결함을 냈고 전부 진짜였다** (`FIX_FIRST` → 수정 → `RUN_AS_WRITTEN`):

| # | 지적 | 확인 | 수정 |
|---|---|---|---|
| C1 | 셸 변수 대입은 **PSReadLine 평문 이력**에 남고, 변수를 지워도 안 지워진다 | 이 머신 실측: `SaveIncrementally`, 316줄 | **에코 끈 TTY 입력**으로 변경 |
| C2 | 출력 상한이 없어 추론 모델이 "한 단어"보다 훨씬 청구할 수 있다 | xAI 레퍼런스: `max_output_tokens`가 추론 토큰까지 상한 | 64로 상한 |
| C3 | 2xx면 "청구됨"으로 출력 — Cloudflare 인터스티셜도 200일 수 있다 | — | `usage.total_tokens > 0`일 때만 billed |
| C4 | 네트워크 예외에 "청구 안 됨"을 **단언** → 재실행해 두 번 낸다 | — | 예외·본문읽기 실패·5xx = **UNKNOWN(exit 2), 재실행 금지** |
| C5 | 원시 HTTP 성공이 **플러그인 트래픽**에 대한 콘솔 검증은 아니다 | — | 주장 축소 — 0은 **"반증 시도 실패"** |

C5가 가장 중요하다. 플러그인은 grok CLI를 거치는 **다른 경로**라 다른 항목으로 청구될 수 있고,
따라서 이 프로브로는 B3가 닫히지 않는다. 요청 경로 자체는 이미 더 강한 증거가 있다
(`auth_type=SessionToken`, 구독 모드의 env 키 삭제).

**배포: 버전 범프·태그·릴리스 없음.** 두 번들이 재빌드 후 **바이트 동일**이고(`git diff` 빈 결과),
`package.json`은 런타임에 **버전 문자열만** 읽히며, 새 스크립트는 엔드유저 환경에서 **실행되지
않는다.** 범프 규칙은 *번들이 바뀌면*이 트리거다. Grok도 같은 결론(`NO_BUMP`)이었고 반대 논거도
정확히 짚었다 — `source: "./"`라 레포 전체가 버전 키 캐시로 복사되므로, 불변식을 엄격히 읽으면
**실행되지 않는 파일의 차이도** 다른 산출물로 볼 수 있다. 그 해석을 택하지 않은 이유는 위 트리거
문장이고, **이 판단을 다시 열기 전에 이 문단부터 읽을 것.**

### 승인받은 변경을 재현 단계에서 중단했다 — "산문 리뷰는 매달린다"가 오늘은 재현되지 않는다

오너가 승인한 작업은 이것이었다: 리뷰 프롬프트 함정("산문 답만 요구하면 헤드리스 grok이 끝나지
않는다 → 프롬프트를 파일 쓰기로 끝내라")이 유지보수자 `CLAUDE.md`에만 있고 **배포 표면
(`commands/`·`skills/`·`agents/`)에는 없으니** 거기에 넣자. 근거는 오너 코퍼스 timeout의 ~93%가
리뷰형이라는 관측이었다.

**절차 1번(배포 번들로 재현)에서 멈췄다. 두 번 시도했고 둘 다 재현되지 않았다.**

| 시도 | 조건 | 결과 |
|---|---|---|
| R1 | 12줄짜리 픽스처 1개, 산문 전용 리뷰, 기본 180초 캡 | **83초 정상 완료**, 답도 정확 |
| R2 | 같은 형태를 **실제 레포**(이 저장소 소스)에 worktree 격리로 | **33초 정상 완료**, 답도 정확 |

코퍼스도 지지하지 않았다. 9월 리뷰형 283건 중 **244건 완료 / 39건 timeout**이고, 종료 지시를
**명시한** 프롬프트의 timeout율이 **35.7%(5/14)** 로 산문 전용 **12.6%(34/269)** 보다 **높다**.
n이 작고 선택편향(첫 시도가 timeout 난 뒤에 그 문구를 덧붙인다)이 있으므로 반증은 아니지만,
**지지는 전혀 아니다.**

Grok에게 "중단이 맞나, 약화판(복구 절차)이라도 낼까"를 물었더니 **ABANDON**이었고, 이유가 정확했다 —
약화판도 "그 복구가 실제로 timeout을 복구한다"는 것을 **어떤 측정도 보여주지 않는다.**

**그래서 아무것도 배포하지 않았다.** 대신 반증을 기록했다: `CLAUDE.md` "5번 조리법"에 반증 주석
(조리법은 유지 — 그렇게 쓴 리뷰는 실제로 빨리 돌아온다 — 다만 "산문이면 매달린다"를 **법칙으로
인용하지 말 것**), `docs/10` B1에 코퍼스 분해 결과.

2026-09-06 측정이 틀렸다는 뜻은 아니다. 그때는 실측이었고, **grok은 스스로 업데이트한다.** 바로
그래서 이 레포는 스냅샷을 법칙으로 쓰지 않는다.

### B1을 코퍼스로 좁혔다 — "편집 런이 매달린다"는 근거가 없다

오너 이력 386건(2026-07-13~09-12)을 분해했다. 문서의 "timeout 17.5%"는 지금 코퍼스에서 9월
41/346 = **11.8%**인데, 그중 **33건이 리뷰형 프롬프트**다. 실제 소스 디렉터리 + 비리뷰만 남기면
6건이고, **그 6건을 하나씩 열어보니 5건도 read-only/claim-review**였다(필터가 `READ-ONLY`·
`Do NOT modify` 표현을 놓쳤다). **진짜 편집 작업 timeout은 1건**이며 그것도 캡 전에 파일 2개를
바꿨다 — 매달린 게 아니라 600초로 부족했던 것이다. 같은 필터로 7월은 **0/26**.

케이스 리뷰를 먼저 하라고 한 것은 Grok이었다. 원안은 편집 위임 20~30회(1~2.5시간 + 쿼터)를
새로 돌리는 것이었는데, **이미 손에 있는 6건을 여는 데는 비용이 0이었고 답이 나왔다.**

### B5(win32 손자 프로세스 정리)를 닫았다 — 답은 "정리된다"가 아니라 **조건부**다

`docs/10` B5는 "timeout kill은 직계 자식만 죽인다. 장수 손자를 띄우는 위임을 캡으로 죽인 뒤
`Win32_Process` 확인"이었다. 네이티브 Windows에서 실제로 쟀다. 모든 측정은 **두 독립 방법**
(프로세스 테이블 + 손자가 1초마다 쓰는 하트비트 파일)으로 읽었다.

- **실제 위임(배포 번들, 60초 캡, 버릴 디렉터리):** grok이 장수 프로세스를 띄운 것을 하트비트로
  확인(런 시작 10초 뒤 등장). 호출은 61.2초에 `timeout`으로 **정상 반환**했고(행 없음),
  캡 이후 **고아 없음** — 프로세스 0개, 하트비트 47행에서 정지.
- **플러그인 kill 경로 복제(에이전트 없이):** 같은 `killTree`를 그대로 복제. non-detached 손자는
  같이 죽었다.
- **한 변수 실험:** 위 복제에서 **손자를 띄우는 쪽의 `detached`만** `true`로. 손자가 **살아남았다**
  (부모 사라진 고아 상태로 계속 하트비트, 손으로 죽여야 했다). A의 spawn 옵션은 바이트 동일 —
  diff로 확인.

**결론:** 플러그인은 손자 정리를 **보장하지 않는다**. `detached` 손자는 캡을 넘어 산다. 실제 위임
1회가 깨끗했던 이유는 **규명하지 않았다** — grok의 spawn 플래그를 읽은 적이 없고 n=1이며 grok은
스스로 업데이트한다. OS 메커니즘도 미규명(주변 Job Object 가설은 반증됨 — 그랬다면 살아남은
detached 손자도 죽었어야 한다). 전문은 `docs/06` "플랫폼 지원 (실측)".

**코드는 바꾸지 않았다.** 사용자에게 관측된 고장이 아니므로 A 항목이 아니다(`repo-scope`). 바꾼다면
win32에서 `taskkill /T /F`로 트리를 거두는 쪽이고, 오너가 목표를 준 뒤의 일이다.

#### 이 측정에서 하네스가 3번, 내 논증이 3번 틀렸다

8(b)가 왜 필수인지 다시 확인됐다. **하네스 오류 3건** — ① 프로세스 매처가 `probe.js`라는 문자열을
담은 **측정용 셸 자신**을 잡았다 ② `node -e` 안에서 `\n`이 진짜 개행으로 접혀 프로브가 구문 오류로
즉사했다(CLAUDE.md가 heredoc에 대해 경고한 그 접힘이 `node -e`에서도 난다 — 처방대로 Write로 파일을
써서 우회) ③ 첫 캡 20초가 grok이 명령을 실행하기도 전이라 측정 자체가 성립하지 않았다. 전부 출력이
명백히 이상해서 잡혔다.

**Grok이 내 논증을 3번 기각했다**(5라운드 만에 `CONCLUSION_SOUND`). ① 변수를 세 개
(`detached`·`stdio`·`unref`) 동시에 바꾼 실험으로 인과를 주장했다 → 한 변수 실험을 다시 시켰고
그게 결정적 증거가 됐다 ② grok의 spawn 플래그를 읽은 적 없이 "grok이 non-detached로 띄운다"고
귀속했다 → 철회 ③ "A의 kill이 원인이 아니다"를 잘못 도출했다 — 복제 실험은 **같은 kill이
non-detached 손자를 같이 데려간다**는 반대 결과였으므로 kill은 여전히 유력한 원인이다 → 원인을
주장하지도 배제하지도 않는 서술로 좁혔다.

한 번은 **Grok이 틀렸다**: 한 변수 실험을 "A가 B를 띄우는 플래그를 바꿨다"고 읽고 기각했는데,
실제로 바뀐 것은 B가 C를 띄우는 플래그였다. 원인은 내 증거 문서가 "whose flag"를 안 적은 것이었고,
diff로 확인한 뒤 문구를 못 박아 다시 물었다. **지적은 행동 전에 실측으로 확인한다**는 규칙이
양쪽 모두에 적용된다.

### v0.2.24 수락의 마지막 칸 — 새 세션이 닫았다, 단 Grok이 두 번 반려한 뒤에

갱신 뒤 **새로 시작된 세션**의 `grok_build_status`가 `serverVersion: 0.2.24`를 반환했다. 이것이
`docs/10` B4가 요구하는 증거이고, v0.2.24 런은 이것으로 열린 칸 없이 끝났다.

**그런데 그 한 줄로 닫으려던 첫 시도를 Grok이 기각했다** (`CLAUDE.md` "5번 조리법"으로 반증 요청):

1. 1라운드 `CLAIM_NOT_CLOSED` — 내용 동일성 증거가 **릴리스 시점의 문서 기록**이라, 지금 도는
   프로세스가 읽은 바이트를 가리키지 않는다. → 캐시 번들 sha256을 **지금** 재서 대조.
2. 2라운드 `CLAIM_NOT_CLOSED` — ① 그 대조 상대가 `origin/main`인데 **태그가 아니다**(릴리스 뒤
   docs 커밋 `10613d1`이 붙어 둘이 갈라져 있었다) ② 시작 시각을 잰 대상이 **자식 MCP 프로세스**이지
   세션이 아니다. → 태그 blob 직접 대조 + `git ls-tree`로 비교 대상 전체 열거 + 부모 claude.exe
   시작 시각.
3. 3라운드 `CLAIM_CLOSED` — "no gap survives".

2라운드 지적 ①이 실제로 중요했다: 태그 `v0.2.24`(`6f2c56f`)와 HEAD(`10613d1`)는 다른 커밋이므로,
`origin/main`과 대조한 것은 **릴리스된 것과 대조한 것이 아니었다**. 결과적으로 두 dist blob은 태그와
HEAD에서 동일 객체(`0306350a…`·`bb092d41…`)였지만, 그것은 확인해야 아는 사실이지 전제가 아니다.

**그래서 수락 기준을 날카롭게 바꿨다** (`docs/09` §5b): `serverVersion` 자기보고는 증거의 시작이고,
함께 재야 하는 것은 ① 캐시 번들의 개행 정규화 sha256 = **태그** blob ② `Win32_Process`로 **세션
프로세스**(자식 MCP가 아니라)의 시작 시각이 캐시 mtime보다 뒤. 실측 전문은 `docs/09` §5.

부수 확인: 같은 캐시 루트에서 `0.2.23` 번들을 도는 node 프로세스 11개가 그대로 살아 있다 — 전부
더 일찍 시작된 claude.exe의 자식. "세션은 시작 시점의 MCP 프로세스를 문다"를 플러그인 밖에서
재측정한 셈이다. Dependabot open 경보는 이 시점 0건.
...
오너 목표(**서비스 안정성·신뢰성 최상 유지**)로 감사를 돌렸다. 라운드마다 질문을 바꿨다 —
표면 / 동작 / 미광고 경로 / 자격증명 / 실패 경로 / 문서 / 릴리스 무결성. 재현된 결함 **2건**이
나왔고 둘 다 배포 번들에서 재현한 뒤 고쳐 **같은 하네스로 다시 쟀다.** 수치와 근거의 원천은
`docs/releases/v0.2.24.md`다 — 여기 옮겨 적지 않는다.

- **A27** `grok_build_usage`의 `total`이 자기 breakdown과 어긋날 수 있었다. 헤드라인 둘이 모두
  `total`로 나누므로 성공률을 조용히 낮게 보고할 수 있었다(잠재 — 오너 실제 326행은 정합).
  미지 값을 버리는 대신 `unknown`으로 센다. 적대적 10사례 중 **4 INCONSISTENT → 0**.
- **A26** 감사 하네스 `mcpcall.mjs`가 서버 사망을 **240초 정지**로 바꿨고, 그보다 짧은 호출자
  타임아웃에서는 서버가 이미 뱉은 진단을 **통째로 잃었다**. `child.on(exit/error)` 추가 →
  **449ms** · exit 1 · 진단 출력. 정상 경로 무변경.

Grok 반증은 두 수정 모두 **`FIX_SOUND`**(A26 5/5, A27 4/4 TRUE). 감사 중 판정 하나는 Grok이
옳고 내가 틀렸다 — "미지 행은 **어떤 경우에도** 성공률을 낮춘다"는 내 진술에 `CLAIM_FALSE`를
냈고, `completed=0`이면 0=0이라는 반례가 맞다. 문구를 "`completed > 0`일 때"로 정정했다.

### 절차 — 8단계(수행 후 검증 + 과정 감사)를 필수로 넣었다

`CLAUDE.md` "작업 수행 방법"에 8단계를 신설했다(오너 지시). (a) 결과 검증: 머지 후 `origin/main`의
**내용**, 캐시·레포 양쪽 `accept-release`, 재현 페이로드 재실행, 새 세션의 `serverVersion`.
(b) **과정 감사**: 모든 findings를 **두 번째 독립 방법으로 재도출한 뒤에만** 행동하고, 하네스가
틀렸던 사실은 CHANGELOG에 남긴다.

**근거는 이 감사 자체다 — 진짜 결함 2건을 찾는 동안 내 측정 하네스가 6번 틀렸다.** heredoc이
역슬래시를 접어 정규식이 죽으면서 실재하는 식별자 25개가 "없음"으로 나왔고(같은 함정에 **두 번**
걸렸다 — 두 번째는 이 규칙을 적는 문장 자체였다), `tail -1`이 사전순이라 `0.2.23` 대신 `0.2.3`을
채점했고, CRLF 바이트차를 번들 불일치로 오인할 뻔했고, grok에 없는 `--prompt`로 게이트를 오판했고,
docs 스윕이 grok user-guide 외부 인용을 죽은 포인터로 세어 후보 10건이 전부 오탐이었다.
**전부 출력이 명백히 이상해서 잡혔을 뿐이다** — 그럴듯하게 틀렸다면 그대로 보고됐을 것이다.

감사가 확인하고 넘어간 것(절대 원칙 1 차등 실측, 프롬프트 플래그 4종 게이트, 문서 죽은 포인터 0건,
릴리스 무결성)은 `docs/releases/v0.2.24.md`에 있다.

### 수락 실행 기록 — v0.2.24 (릴리스 아님) · 8단계 첫 적용

단계별 수치의 원천은 `docs/09` §5 "실행 기록 — v0.2.24"다 — 여기 옮겨 적지 않는다. 요약:
머지 → 즉시 태그·릴리스(Latest) → 클론·설치본 갱신(0.2.24 enabled) → 캐시 번들 = 커밋 blob →
`accept-release` **10/10** → **두 결함의 재현 페이로드를 캐시 번들에 다시 쳐서** 확인 →
실제 349행 이력 정합·`unknown` 0(정상 데이터 무영향).

**새 8단계가 첫 적용에서 곧바로 값을 했다.** 머지 내용을 보는 1차 방법이 `.claude/…` 두 파일을
**MISSING으로 오보**했다 — Git Bash(MSYS)가 `origin/main:.claude/…`를 `originmain;.claude	ools…`로
경로 변환한 것이다. 규칙이 요구하는 대로 **두 번째·세 번째 독립 방법**(`git grep`,
`MSYS_NO_PATHCONV=1`, blob 해시 직접 조회)으로 재도출하니 셋 다 실재를 확인했다. 규칙이 없었다면
이미 머지된 파일을 "떨어졌다"고 판단해 되살리는 작업을 했을 것이다. **이번 세션 하네스 오류 7번째이자,
처음으로 규칙이 잡아낸 건이다.**

Grok에게는 **수락 주장 자체**를 공격시켰다 — 증거 9개·주장 5개 → `ACCEPTANCE_SOUND`(5/5 TRUE).
가장 약한 증거로 "갱신한 세션이 옛 번호를 말한다"를 정확히 골랐다: 그건 산출물에 대해 아무것도
증명하지 않고 규칙을 보여줄 뿐이다.

**남은 칸 하나 —** 갱신 뒤 새로 시작된 세션의 `serverVersion: 0.2.24`. 사람이 아니라 다음 세션이 닫는다.
