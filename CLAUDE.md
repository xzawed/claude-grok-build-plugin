...
- **Phase 1(MVP) 구현 완료.** `mcp-server/`(TypeScript, ESM)가 실제로 존재하며
  ...
  61개가 통과한다(`config` 5, `env` 3, `grok-result` 4, `auth` 9, `delegate` 26,
  `history` 11, `worktree` 2, `smoke` 1). `.claude-plugin/plugin.json`, `.mcp.json`, `commands/*.md`도 존재한다
  (아래 "컴포넌트 지도" 참고).
...
npm test             # vitest run (유닛 테스트 61개)
