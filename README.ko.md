...
| `/grok:status`에 **`grokHomeNote`** 가 보이거나, 로그인했는데도 위임이 로그인하라고 함 | `GROK_HOME`이 상대 경로입니다. grok은 그것을 자신이 실행되는 폴더 — 작업 폴더, `worktree: true`면 새로 만든 worktree — 기준으로 풀고 `~`도 풀지 않습니다. 그래서 폴더마다 grok 홈이 따로 있고, 다른 폴더에서 한 `grok login`은 다른 홈에 들어갔습니다 | `GROK_HOME`을 절대 경로로 설정하고 Claude Code를 재시작하세요. 폴더마다 다른 홈을 원한다면 작업 폴더에서 `grok login`을 실행하세요. worktree 위임은 상대 경로 홈을 쓸 수 없습니다 — 새 worktree마다 세션이 없기 때문입니다 |
