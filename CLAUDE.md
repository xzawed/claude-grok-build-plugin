...
- **`v0.2.24` 수락 — 끝났다. 열린 칸 없음.** 마지막 칸(갱신 뒤 **새로 시작된 세션**의
  `serverVersion`)이 2026-09-12에 닫혔다. 무엇을 어떻게 쟀는지는 `docs/09` §5가 원천이다 —
  여기 옮겨 적지 말 것.
  ⚠️ **다음 릴리스도 마지막 한 칸은 다음 세션 몫이다** — 세션은 **자기가 시작할 때의 MCP 프로세스를
  물고 있어서**, 갱신을 수행한 세션은 갱신 뒤에도 옛 번호를 말한다(`0.2.22`↔`0.2.23`, v0.2.24에서도
  재실측). 증거는 새 세션의 `grok_build_status`뿐이다 (= `/grok:status`의 구동부,
  `commands/status.md`).
  ⚠️ **그런데 `serverVersion` 자기보고 하나로는 닫히지 않는다** — 프로세스가 스스로 고른 문자열이고,
  디렉터리 이름도 바이트 신원이 아니다. 같이 재야 할 둘: ① 캐시 파일의 개행 정규화 sha256 =
  **태그 blob**(`origin/main`이 아니다 — 릴리스 뒤 docs 커밋이 붙으면 둘은 갈라진다) ②
  `Win32_Process`로 **세션 프로세스**(자식 MCP가 아니라)의 시작 시각이 캐시 mtime보다 뒤. 근거와
  절차: `docs/09` §5.
