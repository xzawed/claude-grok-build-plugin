...
| Every `/grok:*` tool refuses, naming `GROK_BUILD_WORKER=1` | That variable marks a process running **inside a Grok worker** this plugin started. grok loads installed Claude plugins into its workers, so the plugin's own server runs there too, and it refuses so a worker can't start a second, unreviewed Grok run. In a Grok transcript this is expected | Only if you see it in your own Claude Code session: that session was launched from a shell carrying the variable — start it from a clean shell |
