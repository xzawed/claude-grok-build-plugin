...
- **차등 비교 (grok 자신이 독립 구현, 쿼터 0):** 손으로 고른 81가지 v0.2.34 불일치 24(거짓 거절 23·거짓 "준비됨" 1)
  ...
  `npm run probe:home`으로 남겼다(실제 폴더 7가지를 더해 728회, 이 소스로 불일치 0).
- **Grok과 함께 — 프롬프트 형태별 결과 (이 작업):**
  ...
  - 행 수를 줄이고 **`effort: low`**로 준 제안(8행)과 메시지 판정(6행)은 각각 3턴에 끝났다 — 둘을 함께 바꿨으니
    ...
    지적 넷을 냈다 — 재시작 안내 누락, 탭을 "공백 문자"로만 부름, 앞 공백에 cmd `&&` 설명, 앞 공백·탭에서 grok은
    세션을 놓치는 게 아니라 실패함. 전부 고쳤다. A35 메모 문장에 대한 1건은 준 사실 밖이라 기각했다(계약 §8에 측정).
