...
**v0.2.18 런 — 아직 돌지 않았다 (2026-09-05 확인).** §4 B가 이 체크리스트를 **릴리즈마다 1회**로
정의하고 v0.2.18은 이미 나갔지만(`gh release list` → Latest `v0.2.18`), 이 머신 설치본은 아직
`0.2.17`이다(`claude plugin list` = Version 0.2.17; 캐시에 0.2.18 디렉터리 없음). 오너가 위
순서로 갱신·재시작한 뒤 2~8단계를 실행하고, 결과를 `### 실행 기록 — v0.2.18` 절로 남긴다.
