...
- **최신 릴리스 `v0.2.25`** (2026-09-13). 무엇이 왜 나갔는지는 `docs/releases/`와
...
- **다음 할 일 (이 레포): 없음.** 여러 차례의 감사가 연 항목은 전부 닫혔다 — **몇 번·몇 건인지는
  ...
  지켜지는가 → **광고하지 않은 경로로도 지켜지는가** → 오래 돌려도 견디는가. 세 번째 질문이 잡은
  것이 A24다(값을 붙여 쓴 `-p`가 인증 게이트와 이력 양쪽에서 동시에 사라졌다).
...
- **`v0.2.25` 수락 — 한 칸 빼고 끝났다. 사람이 할 일은 없다.** 머지 내용 검증(두 방법) → 즉시
  태그·릴리스 → `origin/main` dist blob = 태그 blob → 클론 먼저 설치본 갱신 → 캐시 = 태그 blob →
  `accept-release` 레포·캐시 → **두 결함의 재현 페이로드를 캐시 번들에 재타격**까지 끝났다.
  수치와 실행 기록은 `docs/09` §5가 원천이다 — 여기 옮겨 적지 말 것.
  **남은 한 칸:** 갱신 뒤 **새로 시작된 세션**이 `serverVersion: 0.2.25`를 말하는지.
  `grok_build_status`를 한 번 부르면 끝이고, 사람을 기다릴 필요는 없다.
  ⚠️ **다음 릴리스도 마지막 한 칸은 다음 세션 몫이다** — 세션은 **자기가 시작할 때의 MCP 프로세스를
  물고 있어서**, 갱신을 수행한 세션은 갱신 뒤에도 옛 번호를 말한다(`0.2.22`↔`0.2.23`·v0.2.24에 이어
  v0.2.25에서도 같다). 증거는 새 세션의 `grok_build_status`뿐이다 (= `/grok:status`의 구동부,
  `commands/status.md`).
