...
that `cwd` if status is unavailable). If it carries **`grokHomeNote`**, show it: `GROK_HOME` is relative, so
the Step 4 sample folder has a grok home of its own and may not be signed in.
