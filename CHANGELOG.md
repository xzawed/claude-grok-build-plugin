...
- **무엇:** `env.ts` `grokHome`·`grokHomeFor`가 Windows에서 R1·R2를 적용한 이름을 돌려준다(바꿀 것이 없으면 같은
  ...
  모르는 호출에서도). status의 `nextSteps`는 메모가 있으면 그것을 먼저 가리킨다.
...
- **Grok과 함께 — 프롬프트 형태별 결과 (이 작업):**
  ...
  - 행 수를 줄이고 **`effort: low`**로 준 제안(8행)과 메시지 판정(6행)은 각각 3턴에 끝났다 — 둘을 함께 바꿨으니
    ...
    지적 넷을 냈다 — 재시작 안내 누락, 탭을 "공백 문자"로만 부름, 앞 공백에 cmd `&&` 설명, 끝의 탭이나 드라이브 경로
    앞의 공백이면 grok은 세션을 놓치는 게 아니라 실패함(그렇게 잰 것은 이 두 경우뿐이다). 전부 고쳤다. A35 메모 문장에
...
- **하네스가 틀린 것 (다음 세션이 같은 함정을 밟지 않게):**
  ...
  7. 그렇게 고친 판은 **자기 임시 폴더를 지우지 못했다**(`EBUSY`, 재시도 5회로도). 임시 폴더로 옮겨 간 뒤에 esbuild를
     불러, 계속 살아 있는 esbuild 서비스 프로세스가 그 폴더를 작업 폴더로 쥐었다. 빌드 뒤 `esbuild.stop()`으로 고쳤고,
     남은 폴더는 지웠다.
