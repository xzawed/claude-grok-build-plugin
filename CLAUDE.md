...
- **최신 릴리스 `v0.2.21`** (2026-09-06). 무엇이 왜 나갔는지는 `docs/releases/`와
...
- **사람이 해야 할 미해결: 2건 — (a) PR 머지·태그, (b) 이 머신 설치본 갱신.**
  (a) PR #89(A1~A6, v0.2.20)와 그 위에 쌓인 A7~A20 PR(v0.2.21)을 순서대로 squash-merge 하고,
  **각각 머지 직후** 태그를 끊는다 — 미루면 버전 키 캐시가 오염된다(위 캐시 항목).
  (b) 그 뒤 marketplace update → plugin update → Claude Code 재시작을 돌리고 `/grok:status`의
  `serverVersion`이 `0.2.21`인지 확인한다. 2026-09-06 시점 `claude plugin list` = Version 0.2.17
  (캐시에 `0.2.7`·`0.2.11`·`0.2.17`만), 마켓플레이스 클론 = 0.2.17 — **설치본이 네 버전
  뒤처져 있다.** v0.2.18의 만료 세션 분류도, v0.2.19의 라우터 위험 게이트도, v0.2.20~21의 감사 큐
  수정 20건도 이 머신에서는 아직 돌지 않는다. 그 뒤 `docs/09` §5의 수용 런이 남는다
  (v0.2.21에서 눈으로 볼 네 가지가 거기 적혀 있다).
