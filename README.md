...
| `/grok:status` shows **`grokHomeNote`**, or a delegation still says to log in after you did | `GROK_HOME` is a relative path. grok resolves it against the folder it runs in — the task folder, or a brand-new worktree with `worktree: true` — and does not expand `~` (measured on Windows). So each folder has its own grok home, and a `grok login` run from another folder may have landed in another one | Set `GROK_HOME` to an absolute path — on Windows one that starts with the drive letter, like `C:\…` (grok puts a drive-less `\grok` on the drive of the folder it runs in) — and restart Claude Code. If per-folder homes are what you want, run `grok login` from the task folder. With `worktree: true`, a home inside the new worktree never has a session — use an absolute `GROK_HOME` for worktree runs |
