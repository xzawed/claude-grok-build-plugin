...
| 모든 `/grok:*` 도구가 `GROK_BUILD_WORKER=1`을 들며 거절 | 이 변수는 **이 플러그인이 띄운 Grok 워커 안**의 프로세스라는 표식입니다. grok은 설치된 Claude 플러그인을 워커에 로드하므로 이 플러그인의 서버도 거기서 뜨고, 워커가 검토받지 않는 두 번째 Grok을 띄우지 못하게 거절합니다. Grok 대화 기록에서 보이면 정상입니다 | 내 Claude Code 세션에서 보일 때만: 그 변수가 든 셸에서 세션을 띄운 것입니다 — 깨끗한 셸에서 다시 시작 |
