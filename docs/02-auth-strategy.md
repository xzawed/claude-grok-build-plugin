...
- 세션 토큰은 발급 후 **7일**이 지나면 만료되며, 이후 호출은 재인증을 요구한다
  (구독 모드에만 해당 — API 키는 별도 만료 정책을 따른다).
...
5. **만료/무효 감지는 사후(reactive) 방식.**
   위임 실행이 실패했을 때 stderr/stdout에서 인증 관련 신호(401/403,
   "not authenticated", "login" 키워드 등)를 감지하면 모드별 안내 메시지(구독:
   `grok login` / API: 키 확인)로 전환한다. 사전에 매번 검증하지 않는다.
