...
- **진짜 불일치는 적힌 증상 밖에 있었다.** Node fs는 `\\?\` 경로라 Windows 정규화를 건너뛰고, grok의 조회는 거친다.
  이름이 점·공백으로 끝나는 실제 폴더로 규칙을 좁혔다: **R1** 뒤에 성분이 오는 성분은 끝 점이 정확히 하나일 때
  잃는다(`h.`→`h`, `h..`는 그대로), **R2** 작업 폴더는 마지막 성분 끝의 공백·점을 잃는다(구분자로 끝나면 아니다).
  적어 둔 최소 수정("grok과 같은 정규화" — `du`처럼 다듬기)을 그대로 했다면 끝 공백에서 거짓 "준비됨"을 만들었다.
- **무엇:** `env.ts` `grokHome`·`grokHomeFor`가 Windows에서 R1·R2를 적용한 이름을 돌려준다(바꿀 것이 없으면 같은
  문자열, `\\?\`·POSIX·기본 홈은 그대로). `grokHomeNote`는 `GROK_HOME` 앞뒤 공백을 말한다 — status·auth check
  필드, 거절 메시지, hook deny(폴더를 모르는 호출에서도).
- **차등 비교 (grok 자신이 독립 구현, 쿼터 0):** 손으로 고른 81가지 v0.2.34 불일치 24 → **0**. 생성한
  357가지 × 세션 위치 2곳(714회): v0.2.34 불일치 456 → **0**.
- **Grok과 함께 — 프롬프트 형태별 결과 (이 작업):**
  ...
  - 제안을 8행으로 줄이고 **`effort: low`**: 3턴 만에 끝났다. 8가지 모두 v0.2.34의 진짜 불일치였다 → 표에 넣었다.
...
- **하네스가 틀린 것 (다음 세션이 같은 함정을 밟지 않게):**
  ...
  5. 번들 e2e에 넣은 A36 블록이 서버 호출로 끝나자 `afterAll`의 폴더 삭제가 `EBUSY`로 실패했다 — 죽인 자식이
     아직 폴더를 쥐고 있었다. 헬퍼가 자식의 종료를 기다린 뒤 돌아오게 했다(수정 전 파일로는 3/3 재현 안 됨).
