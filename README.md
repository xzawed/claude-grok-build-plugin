...
Under the hood, `mcp-server/` (TypeScript, ESM) ships nine MCP tools over stdio — auth, **status**, delegate, plan, verify, usage (insights), worktree lifecycle, **route** (recommend only), and cli — plus a PreToolUse auth-check hook. 196 unit tests; prebuilt bundles are committed.
