...
| **`grokHomeNote`** 가 `GROK_HOME` 앞이나 끝의 공백을 말함 | 값에 스페이스·탭·줄바꿈이 들어 있습니다. 흔한 원인은 cmd입니다 — `set GROK_HOME=C:\x && claude`는 `&&` 앞의 공백까지 값에 넣습니다. grok은 그 문자까지 경로로 씁니다: 끝 공백이면 로그인한 폴더가 아니라 이름이 공백으로 끝나는 폴더에서 세션을 찾고, 탭·줄바꿈이나 드라이브 문자 앞의 공백이면 경로가 잘못돼 grok이 아예 실패합니다 | 공백을 지우고(cmd에서는 `set "GROK_HOME=C:\x"`) Claude Code를 재시작하세요 — `grok login`을 다시 하기보다 이것이 먼저입니다 |
