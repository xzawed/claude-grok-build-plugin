...
**최신 릴리스 `v0.2.34`.** 무엇이 왜 나갔는지는 `docs/releases/`와 `CHANGELOG.md`가 원천이다 —
여기 옮겨 적지 말 것. 이 절은 **지금 사실과 함정**만 담고 이력은 담지 않는다.

- **다음 할 일: `docs/10` A36 — 열린 결함 1건, 착수는 오너 승인 후** (`.claude/skills/repo-scope`).
  오너가 목표를 주지 않으면 그 큐 밖의 일은 "없음"이다. 새 결함은 `docs/10-service-audit-queue.md`의 A 섹션에
  적고 **번호는 재사용하지 않는다 — 다음은 A37이다.** 범위 밖(외부/수동/보류)은 `docs/09`.
  기각·반증된 항목을 다시 제기하기 전에 `docs/09`·`docs/releases/`의 근거부터 읽을 것.
- **⚠️ `docs/10`의 B(측정 불가)는 대기열이 아니라 할 일 목록이다.** B4가 "리눅스 머신이 있어야
  잰다"로 닫혀 있었는데 **Docker로 그 머신을 만들고**, 실계정이 필요한 나머지는 **컨테이너 전용 로그인**
  (별도 볼륨의 `GROK_HOME`, 끝나면 로그아웃·삭제)으로 재서 닫았다. 결함(A33)은 커널이 아니라 우리 메시지에
  있었다. 환경을 만들 수 있으면 지금 잰다. 절차는 `docs/specs/grok-cli-contract.md` §13.
- **⚠️ grok CLI는 스스로 업데이트된다.** `1.0.5`→`1.0.13`→`1.0.30`이 **아무도 눈치채지 못한 채**
  ...
  ⚠️ **`drifted`와 `snapshotBehindLatest`는 다른 질문이다 — 합치지 말 것.** 앞은 *이 머신의* CLI가
  스냅샷에서 벗어났는가, 뒤는 *새로 설치하면 받을* 버전이 스냅샷과 다른가다. 2026-09-23에 둘이
  갈라졌다: 채널 포인터가 **1.0.41**인데 이 머신도 스냅샷도 **1.0.30**이라 `drifted: false`였다
  (2026-09-24에 머신을 먼저, 스냅샷을 나중에 1.0.41로 맞췄다 — 순서를 바꾸면 `drifted`가 참이 된다).
  뒤쪽은 **`--strict`를 깨뜨리지 않는다** — grok은 자주 릴리스하므로 그걸로 게이트를 붉히면
  사람들이 무시하게 된다(`b721433`이 고친 실패 모양).
  조회 실패는 `null` + 사유이지 `false`가 아니다 — **"못 물어봤다"는 "최신이다"가 아니다.**
...
- **⚠️ plan 모드의 쓰기 차단 여부는 릴리스마다 뒤집힌다**(1.0.3 막음 → 1.0.13 안 막음 → 1.0.30·1.0.41 막음,
...
> 만들지 않는다".
>
> ⚠️ **왕복도 포함이다 — 내용을 만들지 않아도 깨진다 (2026-09-23, 이 문서가 당했다).**
> `$o = Get-Content f -Raw; …; Set-Content f $o -Encoding utf8`은 아무것도 조립하지 않지만,
> PS 5.1의 읽기 기본 인코딩과 쓰기 인코딩이 달라 **한글 전체가 이중 손상**된다. 따옴표도
> 역슬래시도 개입하지 않으므로 위 근거만 읽으면 안전해 보인다. 원본이 git에 있으면
> `git checkout -- <file>`로 되돌리고, 편집은 Edit/Write로 다시 한다.
...
- ⚠️ **코드를 *추적*하게 하는 주장 — 반례를 만들라는 것 포함 — 은 이 조리법으로도 캡을 넘는다**(grok이 명세를 웹에서
  받아 와 추론을 이어간다). **측정한 사실을 주고 경로마다 *분류*하게 하면 대개 답이 온다** — 캡에 걸리면 같은 세션을
  `resume`해 "이미 한 분석으로 지금 verdict.md를 쓰라"고 한다. 집계와 그 판정이 찾은 결함: `CHANGELOG.md` v0.2.34.
  반례 탐색은 **독립 구현과의 차등 비교**로 하고, Grok에는 후보 *제안*이나 분류를 맡긴다.
