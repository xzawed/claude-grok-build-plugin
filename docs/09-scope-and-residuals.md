...
**이 플러그인 레포의 의도된 제품 범위(다리 + 협업 표면 + first-mile + 소비자 계약/키트)는 완료다 (최신 릴리스 `v0.2.21`).**  
...
**v0.2.18~v0.2.21 런 — 아직 돌지 않았다 (2026-09-06 확인).** §4 B가 이 체크리스트를
**릴리즈마다 1회**로 정의하는데 그 사이 네 번 나갔다. 이 머신 설치본은 아직 `0.2.17`이다
(`claude plugin list` = Version 0.2.17; 캐시에 0.2.18~0.2.21 디렉터리 없음). 오너가 위
순서로 갱신·재시작한 뒤 2~8단계를 실행하고, 결과를 `### 실행 기록 — v0.2.21` 절로 남긴다 —
네 릴리스를 한 번에 수락하는 셈이므로 v0.2.18의 만료 세션 분류, v0.2.19의 라우터 위험 게이트,
그리고 감사 큐 20건(v0.2.20의 A1~A6 · v0.2.21의 A7~A20)을 **전부** 확인한다.

v0.2.21에서 특히 눈으로 볼 것 — 이 넷은 출하 구성에서만 드러나므로 유닛이 대신해 주지 못한다:

1. **hook이 실제로 무장됐는가** (A7). `.mcp.json`에 env가 없는 그대로, 로그아웃 상태에서
   위임을 시도하면 서버가 아니라 **PreToolUse가** 먼저 막아야 한다.
2. **`/grok:status`가 `serverVersion: 0.2.21`을 말하는가** — 아니면 재시작이 안 된 것이다.
3. **`/grok:cli` 오타 한 단어**(예: `sesions`)가 1초 안에 blocked로 돌아오는가 (A11).
4. **`/grok:usage`를 이력 없는 디렉터리에서** 호출했을 때 "이 디렉터리 기준"이라고 말하는가 (A17).
