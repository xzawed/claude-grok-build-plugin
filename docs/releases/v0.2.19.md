...
과차단은 실측으로 확인했다: `drop support for IE11 …`·`rename … across all 40 files`·
`scaffold a CRUD dto` 전부 LOW 유지. `production`은 실제 시스템을 가리킬 때만 잡는다
(`fix the production build config`는 신호가 켜지지 않는다).
...
- `npm test` **397 passed / 1 skipped** (v0.2.18 시점 382 — 회귀 15건 추가)
...
- 라우팅 9케이스 스윕 — 위험 3건 상향, 정상 6건 등급 불변
