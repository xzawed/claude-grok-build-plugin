...
- **사람이 해야 할 미해결: 1건 — Claude Code 재시작 후 `/grok:status` 한 번.**
  2026-09-06에 v0.2.20·v0.2.21을 머지·태그·릴리스했고 설치본도 `0.2.17 → 0.2.21`로 갱신했다
  (`claude plugin list` = Version 0.2.21 · enabled). 새로 캐시된 번들을 직접 띄워 hook 무장·
  serverVersion·A11·A17까지 실측했다 — 기록은 `docs/09` §5 "실행 기록 — v0.2.21".
  **남은 것은 GUI 경로 하나뿐이다**(`docs/10` B4): 갱신 후에도 실행 중이던 세션은 옛 프로세스를
  물고 있어서 이 세션의 `/grok:status`는 아직 0.2.17을 말한다. 재시작하고 한 번 눌러
  `serverVersion: 0.2.21`을 확인하면 닫힌다 — 에이전트는 자기 세션을 재시작할 수 없다.
