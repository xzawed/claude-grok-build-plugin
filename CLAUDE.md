...
- **사람이 해야 할 미해결: 없음 — 단, `v0.2.23` 수락은 아직이다.** 이 버전은 번들이 바뀌었으므로
  머지·태그 뒤 설치본을 갱신하고(`marketplace update` → `plugin update`, 클론이 먼저)
  `node .claude/tools/accept-release.mjs`를 다시 돌려야 한다. 그건 사람 몫이 아니라 **다음 세션
  몫**이다 — 아래 ⚠️가 그 이유이자 방법이다. 직전 `v0.2.22`는 이 절차로 10/10 통과했고, 마지막까지
  열려 있던 GUI 경로(`docs/10` B4)도 재시작된 세션에서 닫혔다 — 실행 기록은 `docs/09` §5.
