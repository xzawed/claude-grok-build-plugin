...
- **⚠️ grok CLI는 스스로 업데이트된다.** `1.0.5`→`1.0.13`→`1.0.30`이 **아무도 눈치채지 못한 채**
  ...
  ⚠️ **probe가 green이라고 계약이 최신인 것은 아니다.** 2026-09-23 실측: 설치 스크립트가
  *"Fetching latest stable version… Installing Grok **1.0.41**"* 이라 말하는 동안 이 머신도
  스냅샷도 **1.0.30**이었고 probe는 `drifted: false`였다. probe가 재는 것은 **이 머신의 CLI가
  스냅샷에서 벗어났는가** 뿐이다(Grok 판정: 헤더는 딱 그만큼만 주장한다). *새 사용자가 받는 것*과
  계약이 맞는지는 **아무도 재고 있지 않다.**
